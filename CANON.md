...
- `CANON-23 --[POST /v1/public-agent-bridge + X-ArcLink-Gateway-Exec-Token; stdin payload]--> CANON-12` (BEV: yes — broker route now preserves bridge delivery evidence; detached job files strip bot tokens and hydrate them at runtime; `arclink_notification_delivery.py`, `arclink_gateway_exec_broker.py`)
...
| S8 | CANON-23 → CANON-12 (bridge) | ~~Detached delivery path writes bot_token to a 0600 job file~~ *[REPAIRED — detached job files strip bot tokens and hydrate from control config at worker runtime.]* | `arclink_notification_delivery.py` |
...
| CANON-12 | ~~Bot token persisted to disk on the detached gateway-exec delivery path~~ *[REPAIRED — job files now strip `bot_token` and rehydrate from runtime config; regression tests assert secrets are absent from job JSON.]* | `arclink_notification_delivery.py` |
...
| CANON-23 | ~~No retry backoff~~ *[REPAIRED/STale — `mark_notification_error` schedules exponential retry and now adds deterministic per-notification jitter.]* | `arclink_control.py` |
...
- **CANON-23:** ~~detached public-agent-turn lease ~2h locks the turn on transient error~~ *[REPAIRED 2026-06-17 — detached bridge workers now record a PID breadcrumb and `reap_orphaned_public_agent_bridge_leases()` re-arms rows whose worker is gone; see `arclink_notification_delivery.py`]*; dashboard/runner disagree on `required_env` (`CLOUDFLARE_API_TOKEN_REF`) `dashboard.py:548`.
...
absence-of-exception~~ *[REPAIRED 2026-06-17 — bridge/broker/worker now require confirmed platform message ids; `unknown` is held as `PROCESSED_UNCONFIRMED_BY_PUBLIC_AGENT_BRIDGE`]*. Helper request timeout clamped (CANON-13 `:494`).
...
8. **CANON-12:** "stdin-only, never on disk" bot-token claim was **false** in the detached path; cap_drop ALL vs setpriv likely breaks agent processes. → **[RECONCILED/UPDATED** — token-to-disk path is now repaired by token-stripped job files + runtime hydration; Verifier's FD-leak REFUTED (log handle GC-closed). cap_drop-vs-setpriv held HIGH pending live proof = **STANDING DISAGREEMENT**; production state-root symlink reality deferred to CANON-13**]**
...
| 23 Diagnostics / Health / Evidence / Notifications / Live Proof | [§](research/canon/sections/CANON-23-diagnostics-health-evidence.md) | [✓](research/canon/verify/CANON-23-diagnostics-health-evidence.verify.md) | 9 | 0/4/2 | Happy-path proven; evidence DB/operator snapshot now wired; split redaction engines were repaired in the prior reconciliation pass; bridge delivery evidence/retry jitter/reaper are source-real. | Run live PG-PUBLIC-AGENT-DELIVERY and full production journey proof. | ✓ both · Codex OBJECT(6) |
...
| 2026-06-17 | CANON-12/CANON-23/CANON-32 | Public Agent bridge D5 repair + cold-spawn posture: bridge success now carries `confirmed`/`unknown`/`failed` delivery evidence with platform message ids; gateway-exec-broker preserves that evidence; notification delivery marks delivered only on confirmed ids, holds unknown rows for reconciliation, jitters retries, records detached worker PIDs, reaps dead-worker leases, and warms Hermes bytecode at deploy. Warm daemon/prefork/sidecar designs are recorded as rejected in `research/bridge-perf-federation-brief.md`. | `PG-PUBLIC-AGENT-DELIVERY`, `PG-BOTS`, `PG-HERMES` | Source/test-real; live Telegram/Discord ack proof still open. |
