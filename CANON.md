...
1. **CANON-30 (HIGH) — token-injection seam break.** `_TOKEN_TOOL_NAMES` is not a superset of token-requiring, agent-advertised MCP tools; `pod_comms.list/send/share-file`, `ssot.approve`, `ssot.deny` are sent with no token and fail closed at the server → a live agent break invisible to CI (`arclink-managed-context/__init__.py:1843` vs `arclink_mcp_server.py:1094,2615,2634`).
2. **CANON-05 (HIGH) — `/credentials` leaks the dashboard password into a public, non-DM/non-ephemeral channel.** The first reveal in a group channel IS the leak (`arclink_public_bots.py:3705-3750`; `arclink_discord.py:469-471`; `arclink_hosted_api.py:2980-2984`).
3. **CANON-13 (HIGH) — production migration verification is effectively non-verifying.** The default verifier reads deployment-keyed health that apply never seeds, accepts empty AND stale source health, ignores captured `docker_status`, and the prod action-worker injects no verifier (`arclink_action_worker.py:1169-1178`; `arclink_pod_migration.py:563-572`).
