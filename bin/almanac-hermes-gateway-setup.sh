...
if [[ -z "$PYTHON_BIN" || ! -x "$PYTHON_BIN" ]]; then
  echo "python3 is required to run Almanac-managed Hermes gateway setup." >&2
  exit 1
fi
...
try:
    ...
    print(
        f"Almanac could not import Hermes gateway internals for managed setup: {exc}",
        file=sys.stderr,
    )
    raise SystemExit(86)
...
def _notify_once() -> None:
    ...
    notified = True
    gw.print_info(
        "  Almanac manages gateway persistence with its own systemd units; "
        "skipping Hermes-native service install/start."
    )


def almanac_prompt_yes_no(question: object, default: bool = True) -> bool:
...
def almanac_install_linux_gateway_from_setup(force: bool = False):
    _notify_once()
    return None, False


gw.prompt_yes_no = almanac_prompt_yes_no
gw.install_linux_gateway_from_setup = almanac_install_linux_gateway_from_setup
gw.gateway_setup()
PY
status=$?
set -e
