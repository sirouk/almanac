...
if [[ "${args[$last_index]}" == "-" ]]; then
  printf 'Example Lattice preflight PDF\n'
else
  printf 'Example Lattice preflight PDF\n' >"${args[$last_index]}"
fi
EOF
    ...
    def do_POST(self):  # noqa: N802
        ...
        response = {
            "choices": [
                {
                    "message": {
                        "content": "- Diagram shows Example Lattice routing and node links.\n- Visual layout adds topology context beyond OCR text."
                    }
                }
            ]
        }
...
run_pdf_ingest_preflight() {
  ...
  local pdf_path="$vault_dir/Inbox/example-lattice-preflight.pdf"
  local generated_md="$state_dir/pdf-ingest/markdown/Inbox/example-lattice-preflight-pdf.md"
...
run_pdf_ingest_vision_preflight() {
  ...
  local state_dir="$priv_dir/state"
  local runtime_dir="$state_dir/runtime"
  local pdf_path="$vault_dir/Inbox/example-lattice-vision.pdf"
  local generated_md="$state_dir/pdf-ingest/markdown/Inbox/example-lattice-vision-pdf.md"
  local status_json="$state_dir/pdf-ingest/status.json"
  ...
  grep -q "Diagram shows Example Lattice routing and node links." "$generated_md"
...
run_vault_watch_preflight() {
  ...
  if ! command -v inotifywait >/dev/null 2>&1; then
    log "inotifywait not present, skipping vault watcher preflight"
    return 0
  fi
...
EOF
  ...
  rm -f "$vault_dir/Inbox/example-lattice-watch.pdf"
...
run_vault_notification_preflight() {
  ...
  if ! command -v inotifywait >/dev/null 2>&1; then
    log "inotifywait not present, skipping watcher notification preflight"
    return 0
  fi
...
run_repo_sync_preflight() {
  ...
  if ! command -v inotifywait >/dev/null 2>&1; then
    log "inotifywait not present, skipping watcher repo-sync preflight"
    return 0
  fi
