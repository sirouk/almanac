...
ARCLINK_MEMORY_SYNTH_MAX_CONTENT_HASH_BYTES="${ARCLINK_MEMORY_SYNTH_MAX_CONTENT_HASH_BYTES:-8388608}"
...
# not available yet; otherwise stale private config must not mask a pin bump.
# Hard-coded fallback values cover the earliest install phase.
# shellcheck disable=SC1091
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/pins.sh" 2>/dev/null || true
__pins_get_or_default() {
  # Usage: __pins_get_or_default <component> <field> <fallback>
  local value
  if declare -f pins_get >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    value="$(pins_get "$1" "$2" 2>/dev/null || true)"
  fi
  if [[ -z "${value:-}" ]]; then
    value="$3"
  fi
