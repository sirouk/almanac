...
ARCLINK_DEFAULT_PRICE_ID="${ARCLINK_DEFAULT_PRICE_ID:-price_arclink_sovereign}"
ARCLINK_SOVEREIGN_PRICE_ID="${ARCLINK_SOVEREIGN_PRICE_ID:-$ARCLINK_DEFAULT_PRICE_ID}"
ARCLINK_SCALE_PRICE_ID="${ARCLINK_SCALE_PRICE_ID:-price_arclink_scale}"
ARCLINK_FIRST_AGENT_PRICE_ID="${ARCLINK_FIRST_AGENT_PRICE_ID:-$ARCLINK_SOVEREIGN_PRICE_ID}"
ARCLINK_ADDITIONAL_AGENT_PRICE_ID="${ARCLINK_ADDITIONAL_AGENT_PRICE_ID:-price_arclink_agent_deployment}"
ARCLINK_SOVEREIGN_MONTHLY_CENTS="${ARCLINK_SOVEREIGN_MONTHLY_CENTS:-9900}"
ARCLINK_SCALE_MONTHLY_CENTS="${ARCLINK_SCALE_MONTHLY_CENTS:-17900}"
ARCLINK_FIRST_AGENT_MONTHLY_CENTS="${ARCLINK_FIRST_AGENT_MONTHLY_CENTS:-9900}"
ARCLINK_ADDITIONAL_AGENT_MONTHLY_CENTS="${ARCLINK_ADDITIONAL_AGENT_MONTHLY_CENTS:-5000}"
...
emit_runtime_config() {
  ...
  fi
  {
    ...
    write_kv ARCLINK_DEFAULT_PRICE_ID "${ARCLINK_DEFAULT_PRICE_ID:-price_arclink_sovereign}"
    write_kv ARCLINK_SOVEREIGN_PRICE_ID "${ARCLINK_SOVEREIGN_PRICE_ID:-${ARCLINK_DEFAULT_PRICE_ID:-price_arclink_sovereign}}"
    write_kv ARCLINK_SCALE_PRICE_ID "${ARCLINK_SCALE_PRICE_ID:-price_arclink_scale}"
    write_kv ARCLINK_FIRST_AGENT_PRICE_ID "${ARCLINK_FIRST_AGENT_PRICE_ID:-${ARCLINK_SOVEREIGN_PRICE_ID:-price_arclink_sovereign}}"
    write_kv ARCLINK_ADDITIONAL_AGENT_PRICE_ID "${ARCLINK_ADDITIONAL_AGENT_PRICE_ID:-price_arclink_agent_deployment}"
    write_kv ARCLINK_SOVEREIGN_MONTHLY_CENTS "${ARCLINK_SOVEREIGN_MONTHLY_CENTS:-9900}"
    write_kv ARCLINK_SCALE_MONTHLY_CENTS "${ARCLINK_SCALE_MONTHLY_CENTS:-17900}"
    write_kv ARCLINK_FIRST_AGENT_MONTHLY_CENTS "${ARCLINK_FIRST_AGENT_MONTHLY_CENTS:-9900}"
    write_kv ARCLINK_ADDITIONAL_AGENT_MONTHLY_CENTS "${ARCLINK_ADDITIONAL_AGENT_MONTHLY_CENTS:-5000}"
...
collect_control_install_answers() {
  ...
  default_first_agent_price_id="${ARCLINK_SOVEREIGN_PRICE_ID:-${ARCLINK_FIRST_AGENT_PRICE_ID:-${ARCLINK_DEFAULT_PRICE_ID:-price_arclink_sovereign}}}"
  default_scale_price_id="${ARCLINK_SCALE_PRICE_ID:-price_arclink_scale}"
  default_additional_agent_price_id="${ARCLINK_ADDITIONAL_AGENT_PRICE_ID:-price_arclink_agent_deployment}"
  ...
  ARCLINK_SOVEREIGN_PRICE_ID="$(normalize_optional_answer "$(ask "Stripe Sovereign price ID ($99/month)" "$default_first_agent_price_id")")"
  if [[ -z "$ARCLINK_SOVEREIGN_PRICE_ID" ]]; then
    ARCLINK_SOVEREIGN_PRICE_ID="price_arclink_sovereign"
  fi
  ARCLINK_FIRST_AGENT_PRICE_ID="$ARCLINK_SOVEREIGN_PRICE_ID"
  if [[ -z "$ARCLINK_FIRST_AGENT_PRICE_ID" ]]; then
    ARCLINK_FIRST_AGENT_PRICE_ID="price_arclink_sovereign"
  fi
  ARCLINK_DEFAULT_PRICE_ID="$ARCLINK_FIRST_AGENT_PRICE_ID"
  ARCLINK_SCALE_PRICE_ID="$(normalize_optional_answer "$(ask "Stripe Scale price ID ($179/month)" "$default_scale_price_id")")"
  if [[ -z "$ARCLINK_SCALE_PRICE_ID" ]]; then
    ARCLINK_SCALE_PRICE_ID="price_arclink_scale"
  fi
  ARCLINK_ADDITIONAL_AGENT_PRICE_ID="$(normalize_optional_answer "$(ask "Stripe Agent Deployment price ID ($50/month)" "$default_additional_agent_price_id")")"
  if [[ -z "$ARCLINK_ADDITIONAL_AGENT_PRICE_ID" ]]; then
    ARCLINK_ADDITIONAL_AGENT_PRICE_ID="price_arclink_agent_deployment"
  fi
  ARCLINK_SOVEREIGN_MONTHLY_CENTS="${ARCLINK_SOVEREIGN_MONTHLY_CENTS:-9900}"
  ARCLINK_SCALE_MONTHLY_CENTS="${ARCLINK_SCALE_MONTHLY_CENTS:-17900}"
  ARCLINK_FIRST_AGENT_MONTHLY_CENTS="${ARCLINK_FIRST_AGENT_MONTHLY_CENTS:-9900}"
