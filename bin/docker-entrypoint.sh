...
ensure_docker_state_dirs() {
  ...
  local -a dirs=(
    ...
    "$PRIV_DIR/state/operator/hermes-home"
    "$PRIV_DIR/state/operator/hermes-home/state"
    "$PRIV_DIR/state/operator/vault"
    "$PRIV_DIR/state/operator/memory"
    "$PRIV_DIR/state/operator/workspace"
    "$PRIV_DIR/state/operator/linked-resources"
    "$PRIV_DIR/state/operator/qmd"
    "$PRIV_DIR/state/operator/secrets"
    "$PRIV_DIR/state/operator/nextcloud/db"
    "$PRIV_DIR/state/operator/nextcloud/redis"
    "$PRIV_DIR/state/operator/nextcloud/html"
...
config_has_placeholder_secrets() {
  ...
  for key in "${keys[@]}"; do
    ...
    case "$value" in
      ""|"''"|\"\"|"change-me"|"'change-me'")
        echo "Docker config secret $key is empty or still set to the placeholder value." >&2
        missing=1
        ;;
    esac
  done
...
write_default_docker_config() {
  ...
  local trusted_proxy_cidrs="${ARCLINK_TRUSTED_PROXY_CIDRS:-172.16.0.0/12}"
  local admin_allowed_cidrs="${ARCLINK_ADMIN_ALLOWED_CIDRS:-}"
  local backend_allowed_cidrs="${ARCLINK_BACKEND_ALLOWED_CIDRS:-172.16.0.0/12}"
  local q_config_repo_dir="" q_config_priv_dir="" q_config_runtime_dir="" q_config_agent_home_root=""
  local q_session_hash_pepper="" q_fleet_enrollment_secret="" q_gateway_exec_broker_token=""
  local q_deployment_exec_broker_token="" q_agent_supervisor_broker_token="" q_operator_upgrade_broker_token=""
  local q_agent_user_helper_token="" q_agent_process_helper_token="" q_migration_capture_helper_token=""
  local q_trusted_host_risk_accepted="" q_postgres_password="" q_nextcloud_admin_password=""
  local q_trusted_proxy_cidrs="" q_admin_allowed_cidrs="" q_backend_allowed_cidrs=""
  local q_docker_host_repo_dir="" q_docker_host_priv_dir=""
  ...
  q_config_repo_dir="$(quote_env_value "$CONFIG_REPO_DIR")"
  q_config_priv_dir="$(quote_env_value "$CONFIG_PRIV_DIR")"
  q_config_runtime_dir="$(quote_env_value "$CONFIG_RUNTIME_DIR")"
  q_config_agent_home_root="$(quote_env_value "$CONFIG_AGENT_HOME_ROOT")"
  q_session_hash_pepper="$(quote_env_value "$session_hash_pepper")"
  q_fleet_enrollment_secret="$(quote_env_value "$fleet_enrollment_secret")"
  q_gateway_exec_broker_token="$(quote_env_value "$gateway_exec_broker_token")"
  q_deployment_exec_broker_token="$(quote_env_value "$deployment_exec_broker_token")"
  q_agent_supervisor_broker_token="$(quote_env_value "$agent_supervisor_broker_token")"
  q_operator_upgrade_broker_token="$(quote_env_value "$operator_upgrade_broker_token")"
  q_agent_user_helper_token="$(quote_env_value "$agent_user_helper_token")"
  q_agent_process_helper_token="$(quote_env_value "$agent_process_helper_token")"
  q_migration_capture_helper_token="$(quote_env_value "$migration_capture_helper_token")"
  q_trusted_host_risk_accepted="$(quote_env_value "$trusted_host_risk_accepted")"
  q_postgres_password="$(quote_env_value "$postgres_password")"
  q_nextcloud_admin_password="$(quote_env_value "$nextcloud_admin_password")"
  q_trusted_proxy_cidrs="$(quote_env_value "$trusted_proxy_cidrs")"
  q_admin_allowed_cidrs="$(quote_env_value "$admin_allowed_cidrs")"
  q_backend_allowed_cidrs="$(quote_env_value "$backend_allowed_cidrs")"
  q_docker_host_repo_dir="$(quote_env_value "${ARCLINK_DOCKER_HOST_REPO_DIR:-}")"
  q_docker_host_priv_dir="$(quote_env_value "${ARCLINK_DOCKER_HOST_PRIV_DIR:-}")"

  cat >"$CONFIG_FILE" <<EOF
# Generated by bin/docker-entrypoint.sh for Docker Compose runtime.
ARCLINK_NAME=arclink
ARCLINK_USER=arclink
ARCLINK_TRUSTED_PROXY_CIDRS=$q_trusted_proxy_cidrs
ARCLINK_ADMIN_ALLOWED_CIDRS=$q_admin_allowed_cidrs
ARCLINK_BACKEND_ALLOWED_CIDRS=$q_backend_allowed_cidrs
ARCLINK_HOME=/home/arclink
ARCLINK_REPO_DIR=$q_config_repo_dir
ARCLINK_PRIV_DIR=$q_config_priv_dir
ARCLINK_PRIV_CONFIG_DIR=$q_config_priv_dir/config
VAULT_DIR=$q_config_priv_dir/vault
STATE_DIR=$q_config_priv_dir/state
NEXTCLOUD_STATE_DIR=$q_config_priv_dir/state/nextcloud
RUNTIME_DIR=$q_config_runtime_dir
PUBLISHED_DIR=$q_config_priv_dir/published
ARCLINK_DB_PATH=$q_config_priv_dir/state/arclink-control.sqlite3
ARCLINK_AGENTS_STATE_DIR=$q_config_priv_dir/state/agents
ARCLINK_CURATOR_DIR=$q_config_priv_dir/state/curator
ARCLINK_CURATOR_MANIFEST=$q_config_priv_dir/state/curator/manifest.json
ARCLINK_CURATOR_HERMES_HOME=$q_config_priv_dir/state/curator/hermes-home
ARCLINK_ARCHIVED_AGENTS_DIR=$q_config_priv_dir/state/archived-agents
ARCLINK_NOTION_INDEX_DIR=$q_config_priv_dir/state/notion-index
ARCLINK_NOTION_INDEX_MARKDOWN_DIR=$q_config_priv_dir/state/notion-index/markdown
...
ARCLINK_PRIVATE_DNS_NAME=
ARCLINK_WIREGUARD_DNS_NAME=
ARCLINK_PRIVATE_MESH_DNS_NAME=
ARCLINK_CONTROL_PRIVATE_BASE_URL=
ARCLINK_WIREGUARD_CONTROL_URL=
ARCLINK_PRIVATE_MESH_CONTROL_URL=
ARCLINK_CONTROL_PRIVATE_BIND_HOST=
ARCLINK_CONTROL_PRIVATE_HTTP_PORT=
ARCLINK_CONTROL_PRIVATE_HTTPS_PORT=443
ARCLINK_WIREGUARD_ENABLED=1
ARCLINK_WIREGUARD_INTERFACE=wg-arclink
ARCLINK_WIREGUARD_NETWORK_CIDR=10.44.0.0/24
ARCLINK_WIREGUARD_CONTROL_IP=10.44.0.1
ARCLINK_WIREGUARD_PORT=51820
ARCLINK_WIREGUARD_ACTIVATE=1
ARCLINK_WIREGUARD_CONTROL_ALLOWED_IPS=10.44.0.1/32
ARCLINK_WIREGUARD_PERSISTENT_KEEPALIVE=25
ARCLINK_WIREGUARD_KEY_PATH=
ARCLINK_WIREGUARD_PUBLIC_KEY_PATH=
ARCLINK_WIREGUARD_CONTROL_PUBLIC_KEY=
ARCLINK_WIREGUARD_CONTROL_ENDPOINT=
ARCLINK_WIREGUARD_WORKER_IP_BASE=10.44.0
ARCLINK_WIREGUARD_NEXT_WORKER_OCTET=11
...
ARCLINK_SESSION_HASH_PEPPER=$q_session_hash_pepper
ARCLINK_SESSION_HASH_PEPPER_REQUIRED=1
ARCLINK_FLEET_ENROLLMENT_SECRET=$q_fleet_enrollment_secret
ARCLINK_GATEWAY_EXEC_BROKER_TOKEN=$q_gateway_exec_broker_token
ARCLINK_DEPLOYMENT_EXEC_BROKER_TOKEN=$q_deployment_exec_broker_token
ARCLINK_AGENT_SUPERVISOR_BROKER_TOKEN=$q_agent_supervisor_broker_token
ARCLINK_OPERATOR_UPGRADE_BROKER_TOKEN=$q_operator_upgrade_broker_token
ARCLINK_AGENT_USER_HELPER_TOKEN=$q_agent_user_helper_token
ARCLINK_AGENT_PROCESS_HELPER_TOKEN=$q_agent_process_helper_token
ARCLINK_MIGRATION_CAPTURE_HELPER_TOKEN=$q_migration_capture_helper_token
ARCLINK_DOCKER_TRUSTED_HOST_RISK_ACCEPTED=$q_trusted_host_risk_accepted
...
ARCLINK_SECRET_STORE_DIR=$q_config_priv_dir/state/sovereign-secrets
...
ARCLINK_CONTROL_HOST_MAX_ARCPOD_SLOTS=2
...
ARCLINK_LLM_ROUTER_FALLBACK_MODELS=
ARCLINK_LLM_ROUTER_FALLBACK_STATUS_CODES=
ARCLINK_LLM_ROUTER_MODEL_REPLACEMENTS=
ARCLINK_LLM_ROUTER_MODEL_AUTO_PROMOTE=1
...
NEXTCLOUD_ADMIN_PASSWORD=$q_nextcloud_admin_password
...
ARCLINK_HERMES_DOCS_STATE_DIR=$q_config_priv_dir/state/hermes-docs-src
ARCLINK_HERMES_DOCS_VAULT_DIR=$q_config_priv_dir/vault/Agents_KB/hermes-agent-docs
...
ARCLINK_DOCKER_AGENT_HOME_ROOT=$q_config_agent_home_root
ARCLINK_DOCKER_CONTAINER_PRIV_DIR=$q_config_priv_dir
ARCLINK_DOCKER_HOST_REPO_DIR=$q_docker_host_repo_dir
ARCLINK_DOCKER_HOST_PRIV_DIR=$q_docker_host_priv_dir
...
ENABLE_QUARTO=0
SEED_SAMPLE_VAULT=1
QUARTO_PROJECT_DIR=$q_config_priv_dir/quarto
QUARTO_OUTPUT_DIR=$q_config_priv_dir/published
EOF
...
if [[ ! -f "$CONFIG_FILE" || "${ARCLINK_DOCKER_REWRITE_CONFIG:-0}" == "1" ]]; then
  if config_file_can_write; then
    ...
    echo "ERROR: unable to write Docker config at $CONFIG_FILE; refusing to start with missing or stale runtime config." >&2
    exit 1
  fi
fi
...
if config_file_can_repair; then
  ...
  if config_has_placeholder_secrets; then
    echo "ERROR: Docker config at $CONFIG_FILE still contains placeholder secrets after repair." >&2
    exit 1
  fi
else
  if config_has_placeholder_secrets; then
    echo "ERROR: unable to repair Docker config secrets at $CONFIG_FILE and one or more required secrets are placeholders." >&2
    exit 1
  fi
  echo "Docker config at $CONFIG_FILE is read-only; required secrets are non-placeholder values." >&2
fi
