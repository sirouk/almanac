...
dump_nextcloud_diagnostics() {
...

  {
    ...
    if nextcloud_using_podman && command -v podman >/dev/null 2>&1; then
      podman pod ps || true
...
apply_vault_acls_for_nextcloud() {
  ...
  done < <(find "$VAULT_DIR" -path '*/.git' -prune -o -type d -print)

  while IFS= read -r path; do
    setfacl -m "$acl_files" "$path"
  done < <(find "$VAULT_DIR" -path '*/.git/*' -prune -o -type f -print)
}
