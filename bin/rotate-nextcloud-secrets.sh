...
rotate_nextcloud_runtime_secrets() {
  ...
  if [[ -n "${NEXTCLOUD_ROTATE_POSTGRES_PASSWORD_FILE:-}" ]]; then
    new_postgres_password="$(read_rotation_secret_file "New Postgres password" "$NEXTCLOUD_ROTATE_POSTGRES_PASSWORD_FILE")"
  fi
  if [[ -n "${NEXTCLOUD_ROTATE_ADMIN_PASSWORD_FILE:-}" ]]; then
    new_admin_password="$(read_rotation_secret_file "New Nextcloud admin password" "$NEXTCLOUD_ROTATE_ADMIN_PASSWORD_FILE")"
  fi
