...
resolve_container_runtime() {
  local requested="${ARCLINK_CONTAINER_RUNTIME:-}"
  ...
  case "$requested" in
    ...
    "")
      ;;
    *)
      echo "Unsupported ARCLINK_CONTAINER_RUNTIME=$requested; expected docker or podman." >&2
      return 1
      ;;
  esac

  if [[ "${ARCLINK_DOCKER_MODE:-0}" == "1" ]] && command -v docker >/dev/null 2>&1; then
    printf '%s\n' docker
    return 0
  fi
...
docker_host_path() {
  local path="$1"
  local container_priv="${ARCLINK_DOCKER_CONTAINER_PRIV_DIR:-${ARCLINK_PRIV_DIR:-/home/arclink/arclink/arclink-priv}}"
  local host_priv="${ARCLINK_DOCKER_HOST_PRIV_DIR:-}"
...
workspace_file = workspace_dir / "arclink.code-workspace"
app_name = f"ArcLink Agent Code ({state.get('unix_user') or 'agent'})"
...
for alias_name in ("ArcLink", "Vault"):
...
if vault_dir and vault_alias != "ArcLink":
    arclink_alias = workspace_home / "ArcLink"
    if not arclink_alias.exists() and not arclink_alias.is_symlink():
        try:
            arclink_alias.symlink_to(vault_dir)
            vault_alias = "ArcLink"
        except OSError:
            pass

vault_container_dir = "/arclink-vault"
open_path = "/workspace"
...
values = {
    ...
    "CONTAINER_NAME": str(state.get("code_container_name") or "arclink-agent-code"),
...
if settings_path.is_file():
    ...
    # Older ArcLink builds seeded this file under the config dir, but
...
if [[ -n "${VAULT_DIR:-}" && -d "$VAULT_DIR" ]]; then
  # Keep the host-level ~/ArcLink symlink valid inside the container too.
  mount_args+=(
...
if [[ "$CODE_SERVER_UID" == "0" && "${ARCLINK_AGENT_CODE_ALLOW_ROOT_CONTAINER:-0}" != "1" ]]; then
  echo "Refusing to run code-server as root against read-write ArcLink mounts. Fix workspace ownership or set ARCLINK_AGENT_CODE_ALLOW_ROOT_CONTAINER=1 for a deliberate local recovery run." >&2
  exit 1
fi
