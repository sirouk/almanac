...
- *Public Agent bridge delivery evidence + cold-spawn performance posture* —
  ...
  (`ARCLINK_BRIDGE_GETME_CACHE`) are implemented with a root-only preload
  wrapper that keeps the real Hermes bridge turn on the normal runtime uid.
  They remain proof-gated and require `PG-PUBLIC-AGENT-DELIVERY` or a named live
  performance gate before any measured runtime speedup claim. Deeper
  cold-import graph reduction remains desirable, but must preserve the same
  spawn boundary.
