...
def _docker_mode() -> bool:
    return str(os.environ.get("ALMANAC_DOCKER_MODE") or "").strip().lower() in {"1", "true", "yes", "on"}
...
def _docker_host_port_is_listening(port: int) -> bool:
    ...
    hosts = [
        host.strip()
        for host in str(os.environ.get("ALMANAC_DOCKER_HOST_PORT_PROBE_HOSTS") or "host.docker.internal").split(",")
        if host.strip()
    ]
    for host in hosts:
        try:
