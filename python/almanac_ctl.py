...
def parse_args() -> argparse.Namespace:
    ...
    retrieval_replay.add_argument("--label", default="almanac-ctl")
...
def _stop_code_container(unix_user: str, container_name: str) -> bool:
    if os.environ.get("ALMANAC_DOCKER_MODE") == "1" and shutil.which("docker") is not None and container_name:
...
def _org_profile_source_path(cfg: Config, raw_path: str) -> Path:
    ...
    from almanac_org_profile import default_profile_path
...
def _org_profile_load_or_exit(cfg: Config, raw_path: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    from almanac_org_profile import load_and_validate
...
def org_profile_preview(cfg: Config, raw_path: str, *, as_json: bool) -> dict[str, Any] | str:
    from almanac_org_profile import format_preview, preview_payload
...
def org_profile_apply(
    ...
    *,
    ...
    from almanac_org_profile import apply_profile, format_preview, preview_payload
...
def org_profile_doctor(conn, cfg: Config, raw_path: str) -> dict[str, Any]:
    from almanac_org_profile import doctor_profile, load_profile
...
def _runuser_qmd_refresh_command(cfg: Config) -> str:
    body = (
        ...
        f"export HOME={shlex.quote(str(cfg.almanac_home))}; "
        'export XDG_CACHE_HOME="$HOME/.cache"; '
        'export NVM_DIR="$HOME/.nvm"; '
        'if [ -s "$NVM_DIR/nvm.sh" ]; then . "$NVM_DIR/nvm.sh"; nvm use --silent default >/dev/null 2>&1 || true; fi; '
        f"export ALMANAC_CONFIG_FILE={shlex.quote(str(os.environ.get('ALMANAC_CONFIG_FILE') or cfg.private_dir / 'config' / 'almanac.env'))}; "
        f"exec {shlex.quote(str(cfg.repo_dir / 'bin' / 'qmd-refresh.sh'))} --skip-embed"
    )
    return f"runuser -u {shlex.quote(cfg.almanac_user)} -- bash -lc {shlex.quote(body)}"
...
def _repair_service_state_file_owner(cfg: Config, path: str) -> None:
    ...
    if not target.exists():
        return
    try:
        service_user = pwd.getpwnam(cfg.almanac_user)
    except KeyError:
        return
    try:
...
def run_retrieval_replay(conn, cfg: Config, args: argparse.Namespace) -> dict[str, Any]:
    ...
    vault_root = _user_home_guess(unix_user) / "Almanac"
...
def main() -> None:
    ...
    with connect_db(cfg) as conn:
        ...
        if args.domain == "internal" and args.action == "curator-refresh":
            ...
            from almanac_pin_upgrade_check import run_detector as run_pin_upgrade_detector
            ...
            try:
                payload = handshake_notion_space(
                    ...
                    token=_secret_arg_value(
                        ...
                        fallback=config_env_value("ALMANAC_SSOT_NOTION_TOKEN", "").strip(),
                        label="token",
                    ),
                    api_version=(
                        args.api_version
