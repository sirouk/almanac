...
def docker_mode_env(cfg: Config) -> dict[str, str]:
    return {
        ...
        "ALMANAC_CONFIG_FILE": str(os.environ.get("ALMANAC_CONFIG_FILE") or cfg.private_dir / "config" / "docker.env"),
        ...
        "ALMANAC_MCP_URL": str(os.environ.get("ALMANAC_MCP_URL") or "http://almanac-mcp:8282/mcp"),
        "ALMANAC_BOOTSTRAP_URL": str(os.environ.get("ALMANAC_BOOTSTRAP_URL") or "http://almanac-mcp:8282/mcp"),
        "ALMANAC_QMD_URL": str(os.environ.get("ALMANAC_QMD_URL") or "http://qmd-mcp:8181/mcp"),
        "RUNTIME_DIR": str(cfg.runtime_dir),
        "HERMES_BUNDLED_SKILLS": str(cfg.runtime_dir / "hermes-agent-src" / "skills"),
        "ALMANAC_DOCKER_HOST_REPO_DIR": str(os.environ.get("ALMANAC_DOCKER_HOST_REPO_DIR") or ""),
...
def docker_host_priv_dir(cfg: Config) -> str:
    host_priv = str(os.environ.get("ALMANAC_DOCKER_HOST_PRIV_DIR") or "").strip()
    if not host_priv:
        raise RuntimeError("ALMANAC_DOCKER_HOST_PRIV_DIR is required for Docker-published agent web surfaces")
    return host_priv
    ...
    env.update(
        {
            ...
            "ALMANAC_AGENT_ID": str(agent["agent_id"]),
        }
    )
    return env
...
def access_state(hermes_home: Path) -> dict[str, Any]:
    path = hermes_home / "state" / "almanac-web-access.json"
    try:
...
def desired_specs(cfg: Config, agent: dict[str, Any], home: Path, hermes_home: Path) -> dict[str, tuple[list[str], dict[str, str], Path]]:
    ...
    if access:
        ...
        if dashboard_proxy_port and dashboard_backend_port:
            root_env = env.copy()
            proxy_container_name = f"almanac-agent-dashboard-proxy-{docker_name(str(agent['agent_id']))}"
            root_env.update(
                {
                    "HOME": "/root",
                    "ALMANAC_DOCKER_CONTAINER_NAME": proxy_container_name,
                }
            )
            specs[f"{agent['agent_id']}:dashboard-proxy"] = (
                [
                    ...
                    "never",
                    "--network",
                    str(root_env["ALMANAC_DOCKER_NETWORK"]),
                    "-p",
                    f"127.0.0.1:{dashboard_proxy_port}:{dashboard_proxy_port}",
                    "-v",
                    f"{docker_host_priv_dir(cfg)}:{root_env['ALMANAC_DOCKER_CONTAINER_PRIV_DIR']}:rw",
                    str(root_env["ALMANAC_DOCKER_IMAGE"]),
                    "python3",
                    str(cfg.repo_dir / "python" / "almanac_basic_auth_proxy.py"),
                    "--listen-host",
                    "0.0.0.0",
                    "--listen-port",
                    dashboard_proxy_port,
                    "--target",
                    f"http://{os.environ.get('ALMANAC_DOCKER_AGENT_SUPERVISOR_HOST') or 'agent-supervisor'}:{dashboard_backend_port}",
                    "--access-file",
                    str(hermes_home / "state" / "almanac-web-access.json"),
                    "--realm",
                    "Almanac Hermes",
                ],
                root_env,
...
def start_process(cfg: Config, key: str, cmd: list[str], env: dict[str, str], cwd: Path) -> subprocess.Popen[str]:
    ...
    log.flush()
    container_name = str(env.get("ALMANAC_DOCKER_CONTAINER_NAME") or "").strip()
    if container_name:
...
def run_provisioner(cfg: Config) -> None:
    ...
    with log_handle(cfg, "enrollment-provisioner") as log:
        try:
            subprocess.run(
                [str(cfg.repo_dir / "bin" / "almanac-enrollment-provision.sh")],
                cwd=str(cfg.repo_dir),
                env=env,
                text=True,
                stdout=log,
...
def main() -> int:
    cfg = Config.from_env()
    poll_seconds = int(os.environ.get("ALMANAC_DOCKER_AGENT_SUPERVISOR_POLL_SECONDS", "10"))
    provision_seconds = int(os.environ.get("ALMANAC_DOCKER_AGENT_PROVISION_SECONDS", "30"))
    refresh_seconds = int(os.environ.get("ALMANAC_DOCKER_AGENT_REFRESH_SECONDS", "14400"))
    cron_seconds = int(os.environ.get("ALMANAC_DOCKER_AGENT_CRON_SECONDS", "60"))
