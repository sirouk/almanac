#!/usr/bin/env python3
"""Run Almanac health and notify the operator when failures change."""
...
FINGERPRINT_KEY = "almanac_health_watch_last_fingerprint"
SUMMARY_KEY = "almanac_health_watch_last_summary"
LAST_NOTIFIED_AT_KEY = "almanac_health_watch_last_notified_at"
...
def _health_command(cfg: Config) -> list[str]:
    configured = str(os.environ.get("ALMANAC_HEALTH_WATCH_HEALTH_CMD") or "").strip()
    if configured:
...
def _config_file_for_child(cfg: Config) -> str:
    ...
    if configured:
        return configured
    candidate = cfg.private_dir / "config" / "almanac.env"
    return str(candidate)


def _run_health(cfg: Config, *, timeout_seconds: int, strict: bool) -> subprocess.CompletedProcess[str]:
    env = dict(os.environ)
    env["ALMANAC_CONFIG_FILE"] = _config_file_for_child(cfg)
    env["ALMANAC_HEALTH_WATCH_CHILD"] = "1"
    env["ALMANAC_HEALTH_STRICT"] = "1" if strict else env.get("ALMANAC_HEALTH_STRICT", "0")
...
def _format_problem_message(status: str, summary: dict[str, Any], fingerprint: str, problem_lines: list[str]) -> str:
    title = "Almanac health-watch detected a failing health check"
    if status == "warn":
        title = "Almanac health-watch detected health warnings"
    lines = [
        title,
        f"Time: {utc_now_iso()}",
...
def _format_recovery_message(summary: dict[str, Any], previous_fingerprint: str) -> str:
    return "\n".join(
        [
            "Almanac health-watch recovered",
            f"Time: {utc_now_iso()}",
            f"Summary: {summary.get('line') or 'health summary unavailable'}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Almanac health and notify the operator on changed failures.")
    parser.add_argument("--timeout-seconds", type=int, default=int(os.environ.get("ALMANAC_HEALTH_WATCH_TIMEOUT_SECONDS", "300")))
    parser.add_argument("--strict", action="store_true", default=_bool_env("ALMANAC_HEALTH_WATCH_STRICT", False))
    parser.add_argument(
        "--notify-warnings",
        action="store_true",
        default=_bool_env("ALMANAC_HEALTH_WATCH_NOTIFY_WARNINGS", False),
    )
