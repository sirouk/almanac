...
TOOLS = {
    ...
    "notion.query": "Run a live structured query against a shared Notion database/data source. Prefer when the user's question maps to structured columns on the database (people-typed ownership channels by any name, status/state/priority, due/last-edited dates, or other filterable fields) — pick the columns the user's framing implies rather than a fixed keyword list. For broad knowledge lookup by phrase, call notion.search-and-fetch.",
