...
TOOLS = {
    ...
    "ssot.write": "Apply a Notion SSOT write (insert/update/append/create_page/create_database) through the central broker. Use create_page or create_database for org-wide pages, lists, and task tables so they are parented under the shared Almanac page and inherit org access. Out-of-scope writes queue for user approval; applied page/database creates promote url/id as receipts. Archive/delete are rejected. For cross-turn follow-up on a queued write, call ssot.status.",
    ...
    "ssot.write": _schema(
        {
            ...
            "target_id": {"type": "string", "description": "Required for append/update. For insert, use the parent page/database/data-source id or URL when the configured SSOT target is not enough. For create_page/create_database, use the shared parent page id/URL or omit to use the configured Almanac root page."},
            "payload": SSOT_PAYLOAD_PROP,
...
def _vault_source_path_for_qmd_ref(cfg: Config, source_ref: str) -> tuple[str, str, Path | None, dict[str, str]]:
    ...
    if collection == "vault-pdf-ingest":
        ...
        if not file_metadata:
            file_metadata = _pdf_manifest_metadata_for_rel_path(cfg, rel_path)
