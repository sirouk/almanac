#!/usr/bin/env python3
"""almanac_pin_upgrade_check — detect upstream upgrades for pinned components
and notify the operator with a digest, throttled per release.

Runs hourly via almanac-curator-refresh.timer (also callable via
...
def _import_almanac_control():
    """Import almanac_control without circular imports. The detector is
    callable both standalone and from almanac_ctl.py.
    """
    sys.path.insert(0, str(REPO_ROOT / "python"))
    import almanac_control  # noqa: E402  (late import on purpose)
    return almanac_control
...
def _github_raw_text(repo: str, ref: str, path: str) -> str:
    ...
    request = Request(url, headers={"User-Agent": "almanac-pin-upgrade-check"})
    try:
...
def _ensure_state_table(conn: sqlite3.Connection) -> None:
    # Schema is created by almanac_control.ensure_schema. This function is
...
def run_detector(conn: sqlite3.Connection, cfg: Any) -> dict[str, Any]:
    ...
    if included:
        ...
        # in the full almanac_control import graph.
        ac = _import_almanac_control()
        operator_target = (
