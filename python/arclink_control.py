...
def active_deploy_operation(cfg: "Config") -> dict[str, str] | None:
    ...
    if operation not in {
        ...
        "control-upgrade",
        "control-backup",
        "control-reset-sandbox",
        "control-reset-production",
    }:
        return None
...
ARCLINK_PREFIX_CODE_ALPHABET = "23456789abcdefghjkmnpqrstuvwxyz"
ARCLINK_PREFIX_CODE_LENGTH = 4
...
def _arclink_rng_code(rng: Any | None, *, length: int = ARCLINK_PREFIX_CODE_LENGTH) -> str:
    return "".join(_arclink_rng_choice(ARCLINK_PREFIX_CODE_ALPHABET, rng) for _ in range(max(1, int(length))))
...
def build_managed_memory_payload(
    ...
    *,
    ...
    vault_ref = (
        f"Workspace root: {vault_root}\n"
        f"Vault root: {vault_root}\n"
        "Drive and Code expose Workspace, Fleet, and Linked as the top-level roots.\n"
        "Workspace is this agent's own writable work area, owned by this single agent so active work is not duplicated or conflicted across the Crew: keep Workspace/Projects for project work, Workspace/Repos for code checkouts, and Workspace/Research for agent-local research here, not in Fleet.\n"
        "Fleet is the Captain's shared read/write libraries for the Crew, synced across ArcPods and machines; use it ONLY for material the whole Crew should share: Fleet/Agents_KB (shared knowledge base), Fleet/Agents_Skills (reusable skills), and Fleet/Agents_Plugins (shared plugins). Do not put Projects, Repos, or Research in Fleet — those live in one agent's Workspace at a time.\n"
        "Linked contains accepted folders shared from others; read/write shares can be edited there, but Linked resources cannot be reshared and git mutations are blocked.\n"
        "Use this user-visible path for Workspace/vault file references in chat and VS Code.\n"
    ...
    qmd_ref = (
        ...
        "qmd indexes text-like files anywhere under the Workspace/vault root;\n"
