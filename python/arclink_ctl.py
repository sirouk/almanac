...
from arclink_agent_access import clear_tailscale_https, load_access_state
from arclink_notion_ssot import (
    DEFAULT_NOTION_API_VERSION,
    handshake_notion_space,
    preflight_notion_root_children,
    retrieve_notion_user,
)
from arclink_nextcloud_access import delete_nextcloud_user_access
from arclink_notion_webhook import (
...
from arclink_onboarding_flow import notify_session_state, send_session_message
from arclink_control import (
    ...
    publish_central_managed_memory,
    queue_notification,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Operator CLI for ArcLink control-plane state.")
    ...
    revoke.add_argument("--reason", default="revoked via arclink-ctl")
    ...
    retrieval_sub = retrieval.add_subparsers(dest="action", required=True)
    retrieval_replay = retrieval_sub.add_parser("replay")
    retrieval_replay.add_argument("--agent-id", default="", help="Agent id to replay against. Defaults to --unix-user lookup.")
    retrieval_replay.add_argument("--unix-user", default="", help="Unix user whose vault/token should be used.")
    retrieval_replay.add_argument("--include-notion", action="store_true", help="Seed the notion-shared index and replay the Notion rail too.")
    retrieval_replay.add_argument("--bulk-count", type=int, default=40)
    retrieval_replay.add_argument("--timeout", type=float, default=90.0)
    retrieval_replay.add_argument("--poll-interval", type=float, default=0.25)
    retrieval_replay.add_argument("--run-id", default="")
    retrieval_replay.add_argument("--keep-data", action="store_true")
    retrieval_replay.add_argument("--no-pdf", action="store_true")
    retrieval_replay.add_argument("--scorecard-file", default="", help="JSONL scorecard path. Defaults to state/retrieval-replay-scorecard.jsonl.")
    retrieval_replay.add_argument("--no-scorecard", action="store_true")
    retrieval_replay.add_argument("--label", default="arclink-ctl")

    org_profile = subparsers.add_parser("org-profile")
    org_profile_sub = org_profile.add_subparsers(dest="action", required=True)
    for action in ("validate", "preview", "doctor"):
        command = org_profile_sub.add_parser(action)
        command.add_argument("--file", default="", help="Profile YAML path. Defaults to arclink-priv/config/org-profile.yaml.")
    org_build = org_profile_sub.add_parser("build")
    org_build.add_argument("--file", default="", help="Profile YAML path. Defaults to arclink-priv/config/org-profile.yaml.")
    ...
    org_apply.add_argument("--file", default="", help="Profile YAML path. Defaults to arclink-priv/config/org-profile.yaml.")
    org_apply.add_argument("--yes", action="store_true", help="Apply without an interactive confirmation prompt.")
    org_apply.add_argument("--actor", default=os.environ.get("USER", "operator"))
    ...
    internal_sub.add_parser("pin-upgrade-check")
    ...
    notion_webhook_status.add_argument("--show-secret", action="store_true", help="Reveal the currently stored verification token. Requires root or ownership of the ArcLink control files.")
    ...
    notion_handshake.add_argument("--token-file", default="", help="Read the Notion integration token from this file, or '-' for stdin.")
    ...
    notion_preflight.add_argument("--token-file", default="", help="Read the Notion integration token from this file, or '-' for stdin.")
    ...
    notion_suspend.add_argument("--reason", default="identity suspended via arclink-ctl")
    ...
    notion_unsuspend.add_argument("--reason", default="identity unsuspended via arclink-ctl")
    ...
    for provision_name in ("provision", "provisions"):
        ...
        cancel.add_argument("--reason", default="cancelled via arclink-ctl")
...
def require_control_file_owner(cfg: Config, message: str) -> None:
    ...
    raw_config_path = str(os.environ.get("ARCLINK_CONFIG_FILE", "") or "").strip()
    if raw_config_path:
...
def _manual_verify_notion_user(user_id: str, *, expected_email: str = "") -> tuple[str, str]:
    token = str(config_env_value("ARCLINK_SSOT_NOTION_TOKEN", "") or "").strip()
    if not token:
        raise SystemExit("shared Notion SSOT integration secret is not configured")
    api_version = (
        str(config_env_value("ARCLINK_SSOT_NOTION_API_VERSION", DEFAULT_NOTION_API_VERSION) or "").strip()
...
def _stop_code_container(unix_user: str, container_name: str) -> bool:
    if os.environ.get("ARCLINK_DOCKER_MODE") == "1" and shutil.which("docker") is not None and container_name:
        result = _run_quiet(["docker", "rm", "-f", container_name])
...
def user_prepare(cfg: Config, unix_user: str) -> dict:
    require_root("arclink-ctl user prepare must run as root.")
    ...
    return {
        ...
        "external_steps": [
            ...
            "confirm any tailnet identity mapping or host-access policy outside ArcLink",
        ],
    }


def user_sync_access(cfg: Config, unix_user: str, agent_id: str) -> dict:
    require_root("arclink-ctl user sync-access must run as root.")
    return grant_agent_runtime_access(cfg, unix_user=unix_user, agent_id=agent_id)


def _org_profile_source_path(cfg: Config, raw_path: str) -> Path:
    if raw_path:
        return Path(raw_path).expanduser().resolve()
    from arclink_org_profile import default_profile_path

    return default_profile_path(cfg)


def _org_profile_load_or_exit(cfg: Config, raw_path: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    from arclink_org_profile import load_and_validate

    source_path = _org_profile_source_path(cfg, raw_path)
    try:
        profile, validation = load_and_validate(source_path, cfg)
    except Exception as exc:  # noqa: BLE001
        raise SystemExit(f"org-profile load failed for {source_path}: {exc}") from exc
    return source_path, profile, validation


def org_profile_validate(cfg: Config, raw_path: str) -> dict[str, Any]:
    source_path, _profile, validation = _org_profile_load_or_exit(cfg, raw_path)
    return {
...
def org_profile_preview(cfg: Config, raw_path: str, *, as_json: bool) -> dict[str, Any] | str:
    from arclink_org_profile import format_preview, preview_payload

    source_path, profile, _validation = _org_profile_load_or_exit(cfg, raw_path)
    payload = preview_payload(profile, cfg=cfg, source_path=source_path)
    return payload if as_json else format_preview(payload)


def org_profile_apply(
    conn,
    cfg: Config,
    raw_path: str,
    *,
    ...
    from arclink_org_profile import apply_profile, format_preview, preview_payload

    source_path, profile, validation = _org_profile_load_or_exit(cfg, raw_path)
    if not validation.get("valid"):
        return {"applied": False, "source": str(source_path), **validation}
    if not yes:
        if not sys.stdin.isatty():
            raise SystemExit("org-profile apply requires --yes when stdin is not interactive")
        print(format_preview(preview_payload(profile, cfg=cfg, source_path=source_path)))
        answer = input("Apply this org profile? [y/N]: ").strip().lower()
        if answer not in {"y", "yes"}:
            return {"applied": False, "source": str(source_path), "reason": "operator declined"}
    ...
    for row in refresh_rows:
        ...
        if not agent_id:
            continue
        try:
            publish_result = publish_central_managed_memory(conn, cfg, agent_id=agent_id)
            ...
            refresh_results.append(
                {
                    "agent_id": agent_id,
                    **publish_result,
                    "activation_trigger_path": str(trigger_path) if trigger_path is not None else "",
                }
            )
        except Exception as exc:  # noqa: BLE001
            refresh_failures.append(f"{agent_id}: {exc}")
    report["managed_memory_refresh"] = refresh_results
    report["refresh_failures"] = refresh_failures
    return report


def org_profile_doctor(conn, cfg: Config, raw_path: str) -> dict[str, Any]:
    from arclink_org_profile import doctor_profile, load_profile
...
def org_profile_build(cfg: Config | None, args: argparse.Namespace) -> None:
    if args.file:
        ...
        source_path = Path(__file__).resolve().parents[1] / "arclink-priv" / "config" / "org-profile.yaml"
    command = [
...
def user_purge_enrollment(
    ...
    *,
    ...
    require_root("arclink-ctl user purge-enrollment must run as root.")
    if unix_user == cfg.arclink_user:
        raise SystemExit(f"refusing to purge the ArcLink service user: {unix_user}")
    ...
    with connect_db(cfg) as conn:
        ...
        repo_checkout = cfg.state_dir / "repo-sync" / "checkouts" / f"{unix_user}-arclink"
        if _remove_path(repo_checkout):
            removed_paths.append(str(repo_checkout))
        repo_mirror = cfg.vault_dir / "Repos" / "_mirrors" / f"{unix_user}-arclink"
...
def agent_deenroll(cfg: Config, target: str, actor: str) -> dict:
    require_root("arclink-ctl agent deenroll must run as root.")
    with connect_db(cfg) as conn:
...
def register_curator(cfg: Config, args: argparse.Namespace) -> dict:
    from arclink_control import register_agent
...
def _noninteractive_git_env(env: dict[str, str] | None = None) -> dict[str, str]:
    merged = os.environ.copy()
    if env:
        merged.update(env)
    merged["GIT_TERMINAL_PROMPT"] = "0"
    merged["GIT_ASKPASS"] = "/bin/false"
    merged["SSH_ASKPASS"] = "/bin/false"
    merged["GCM_INTERACTIVE"] = "Never"
    merged.setdefault("GIT_SSH_COMMAND", "ssh -o BatchMode=yes -o ConnectTimeout=15")
    return merged
...
def _query_upstream_head(repo_url: str, branch: str, env: dict[str, str] | None = None) -> str:
    ...
    result = subprocess.run(
        ...
        cwd="/",
        timeout=30,
        env=_noninteractive_git_env(env),
    )
    if result.returncode != 0:
...
def _secret_arg_value(*, value: str = "", file_path: str = "", fallback: str = "", label: str = "secret") -> str:
    if value and file_path:
        raise SystemExit(f"choose only one of --{label} or --{label}-file")
    if file_path:
        try:
            return (sys.stdin.read() if file_path == "-" else Path(file_path).read_text(encoding="utf-8")).strip()
        except OSError as exc:
            raise SystemExit(f"cannot read --{label}-file: {exc}") from exc
    ...
    *,
    ...
    try:
        ...
        if not upstream_commit:
            ...
            note_refresh_job(
                conn,
                job_name="arclink-upgrade-check",
    ...
    upsert_setting(conn, "arclink_upgrade_last_seen_sha", upstream_commit)
    upsert_setting(conn, "arclink_upgrade_relation", relation)

    if notify and update_available:
        last_notified_sha = get_setting(conn, "arclink_upgrade_last_notified_sha", "")
            ...
            if deploy_operation is not None:
                ...
                for row in conn.execute(
                    ...
                    queue_notification(
                        ...
                        target_id=agent_id,
                        channel_kind="arclink-upgrade",
                        message=(
                            "Curator reports an ArcLink host update is available: "
                ...
                upsert_setting(conn, "arclink_upgrade_last_notified_sha", upstream_commit)
...
def _resolve_retrieval_replay_agent(conn, args: argparse.Namespace) -> dict[str, Any]:
    target = str(args.agent_id or args.unix_user or "").strip()
    if target:
        agent = get_agent(conn, target)
        if agent is None:
            raise SystemExit(f"unknown agent or unix user: {target}")
        return agent

    active = [
        row
        for row in list_agents(conn)
        if str(row.get("role") or "") == "user" and str(row.get("status") or "") == "active"
    ]
    if len(active) == 1:
        return active[0]
    raise SystemExit("retrieval replay requires --agent-id or --unix-user when more than one user agent is active")


def _runuser_qmd_refresh_command(cfg: Config) -> str:
    body = (
        "unset QMD_INDEX_DB_PATH INDEX_PATH; "
        f"export HOME={shlex.quote(str(cfg.arclink_home))}; "
        'export XDG_CACHE_HOME="$HOME/.cache"; '
        'export NVM_DIR="$HOME/.nvm"; '
        'if [ -s "$NVM_DIR/nvm.sh" ]; then . "$NVM_DIR/nvm.sh"; nvm use --silent default >/dev/null 2>&1 || true; fi; '
        f"export ARCLINK_CONFIG_FILE={shlex.quote(str(os.environ.get('ARCLINK_CONFIG_FILE') or cfg.private_dir / 'config' / 'arclink.env'))}; "
        f"exec {shlex.quote(str(cfg.repo_dir / 'bin' / 'qmd-refresh.sh'))} --skip-embed"
    )
    return f"runuser -u {shlex.quote(cfg.arclink_user)} -- bash -lc {shlex.quote(body)}"


def _repair_service_state_file_owner(cfg: Config, path: str) -> None:
    if not path:
        return
    target = Path(path)
    if not target.exists():
        return
    try:
        service_user = pwd.getpwnam(cfg.arclink_user)
    except KeyError:
        return
    try:
...
def run_retrieval_replay(conn, cfg: Config, args: argparse.Namespace) -> dict[str, Any]:
    agent = _resolve_retrieval_replay_agent(conn, args)
    unix_user = str(agent.get("unix_user") or "").strip()
    if not unix_user:
        raise SystemExit(f"agent {agent.get('agent_id') or ''} has no unix_user")
    hermes_home = Path(str(agent.get("hermes_home") or "")).expanduser()
    token_file = hermes_home / "secrets" / "arclink-bootstrap-token"
    if not token_file.is_file():
        raise SystemExit(f"bootstrap token file is missing for {unix_user}: {token_file}")
    vault_root = _user_home_guess(unix_user) / "ArcLink"
    tool_repo_dir = Path(__file__).resolve().parents[1]
    script = tool_repo_dir / "bin" / "retrieval-journey-replay.py"
    if not script.is_file():
        raise SystemExit(f"retrieval replay script missing: {script}")
    ...
    if args.include_notion:
        refresh_command = _runuser_qmd_refresh_command(cfg)
        command.extend(
            [
                "--notion-index-root",
                str(cfg.state_dir / "notion-index" / "markdown"),
                "--after-seed-command",
                refresh_command,
    ...
    if not args.no_scorecard:
        scorecard_file = str(args.scorecard_file or (cfg.state_dir / "retrieval-replay-scorecard.jsonl"))
        command.extend(["--scorecard-file", scorecard_file])

    env = os.environ.copy()
    env["PYTHONPATH"] = f"{tool_repo_dir / 'python'}{os.pathsep}{env['PYTHONPATH']}" if env.get("PYTHONPATH") else str(tool_repo_dir / "python")
    result = subprocess.run(
        command,
        capture_output=True,
        text=True,
        check=False,
        env=env,
        cwd=str(tool_repo_dir),
        timeout=max(60, int(float(args.timeout) * 12) + 120),
    )
    try:
        report = json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        raise SystemExit(
            "retrieval replay did not emit JSON"
            + (f"\nstdout:\n{result.stdout[-4000:]}" if result.stdout else "")
            + (f"\nstderr:\n{result.stderr[-4000:]}" if result.stderr else "")
        ) from exc
    if not isinstance(report, dict):
        raise SystemExit("retrieval replay emitted non-object JSON")
    _repair_service_state_file_owner(cfg, scorecard_file)
    report["agent_id"] = str(agent.get("agent_id") or "")
    report["unix_user"] = unix_user
...
def main() -> None:
    ...
    if args.domain == "org-profile" and args.action == "validate":
        dump_output(args, org_profile_validate(cfg, args.file))
        return
    ...
    with connect_db(cfg) as conn:
        ...
        if args.domain == "org-profile" and args.action == "apply":
            ...
            return
        if args.domain == "org-profile" and args.action == "doctor":
            dump_output(args, org_profile_doctor(conn, cfg, args.file))
            return
        ...
        if args.domain == "retrieval" and args.action == "replay":
            report = run_retrieval_replay(conn, cfg, args)
            dump_output(args, report)
            if not report.get("ok") or int(report.get("command_returncode") or 0) != 0:
                raise SystemExit(1)
            return
        ...
        if args.domain == "channel" and args.action == "reconfigure":
            ...
            if platform == "discord":
                ...
                if discord_target_kind == "channel":
                    ...
                    if discord_bot_token:
                        discord_candidates.append(("arclink.env", discord_bot_token))
                    ...
                    if not discord_candidates:
                        raise SystemExit(
                            ...
                            "or set it in arclink.env before running this command"
                        )
                ...
                if telegram_bot_token:
                    telegram_candidates.append(("arclink.env", telegram_bot_token))
            ...
            if platform != "tui-only":
                from arclink_notification_delivery import (
                    deliver_discord,
                    deliver_discord_channel,
                    deliver_telegram,
                )

                test_msg = f"arclink-ctl channel reconfigure operator test ping at {utc_now_iso()}"
                if platform == "discord":
            ...
            configured_path = str(os.environ.get("ARCLINK_CONFIG_FILE") or "").strip()
            config_path = Path(configured_path) if configured_path else cfg.private_dir / "config" / "arclink.env"
            config_updates = {
        ...
        if args.domain == "internal" and args.action == "curator-refresh":
            ...
            from arclink_pin_upgrade_check import run_detector as run_pin_upgrade_detector
            ...
            try:
                pin_upgrade = run_pin_upgrade_detector(conn, cfg)
            except Exception as exc:  # noqa: BLE001
                pin_upgrade = {"ok": False, "error": str(exc)}
            note_refresh_job(
                conn,
                ...
                note=(
                    ...
                    f"pin-upgrade notified: {bool(pin_upgrade.get('notified'))}"
                ),
            )
            dump_output(args, {"repo_sync": repo_sync, "scan": scan, "notion_index": notion_index, "fanout": fanout, "upgrade": upgrade, "pin_upgrade": pin_upgrade})
            return
        if args.domain == "internal" and args.action == "pin-upgrade-check":
            from arclink_pin_upgrade_check import run_detector as run_pin_upgrade_detector
            dump_output(args, run_pin_upgrade_detector(conn, cfg))
            return
        ...
        if args.domain == "notion" and args.action == "webhook-status":
            ...
            if args.show_public_url:
                payload["public_url"] = str(config_env_value("ARCLINK_NOTION_WEBHOOK_PUBLIC_URL", "") or "").strip()
            if bool(args.show_secret):
                require_control_file_owner(
                    cfg,
                    "arclink-ctl notion webhook-status --show-secret must run as root or the owner of the ArcLink control files.",
                )
        ...
        if args.domain == "notion" and args.action == "webhook-arm-install":
            require_control_file_owner(
                cfg,
                "arclink-ctl notion webhook-arm-install must run as root or the owner of the ArcLink control files.",
            )
            dump_output(
                args,
                arm_verification_token_install(
        ...
        if args.domain == "notion" and args.action == "webhook-confirm-verified":
            require_control_file_owner(
                cfg,
                "arclink-ctl notion webhook-confirm-verified must run as root or the owner of the ArcLink control files.",
            )
            dump_output(
                args,
                mark_verification_token_verified(
        ...
        if args.domain == "notion" and args.action == "webhook-reset-token":
            require_control_file_owner(
                cfg,
                "arclink-ctl notion webhook-reset-token must run as root or the owner of the ArcLink control files.",
            )
        ...
        if args.domain == "notion" and args.action == "handshake":
            try:
                payload = handshake_notion_space(
                    space_url=args.space_url or config_env_value("ARCLINK_SSOT_NOTION_SPACE_URL", "").strip(),
                    token=_secret_arg_value(
                        value=args.token,
                        file_path=args.token_file,
                        fallback=config_env_value("ARCLINK_SSOT_NOTION_TOKEN", "").strip(),
                        label="token",
                    ),
                    api_version=(
                        args.api_version
                        or config_env_value("ARCLINK_SSOT_NOTION_API_VERSION", DEFAULT_NOTION_API_VERSION).strip()
                        or DEFAULT_NOTION_API_VERSION
                    ),
                )
        ...
        if args.domain == "notion" and args.action == "preflight-root":
            try:
                payload = preflight_notion_root_children(
                    root_page_id=(
                        args.root_page_id
                        or config_env_value("ARCLINK_SSOT_NOTION_ROOT_PAGE_ID", "").strip()
                        or config_env_value("ARCLINK_SSOT_NOTION_SPACE_ID", "").strip()
                    ),
                    token=_secret_arg_value(
                        value=args.token,
                        file_path=args.token_file,
