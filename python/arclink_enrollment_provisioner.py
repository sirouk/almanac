...
from arclink_notion_ssot import retrieve_notion_page
from arclink_nextcloud_access import sync_nextcloud_user_access
from arclink_onboarding_provider_auth import (
...
def _operator_sqlite_retry_seconds() -> float:
    raw = str(os.environ.get("ARCLINK_OPERATOR_SQLITE_RETRY_SECONDS") or "180").strip()
    try:
        value = float(raw)
    except ValueError:
        return 180.0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Provision ArcLink enrollments and self-serve claim flow.")
    parser.add_argument(
        "--claims-only",
        action="store_true",
...
def _config_file_for_child_process(cfg: Config) -> str:
    ...
    if configured:
        return configured
    return str(cfg.private_dir / "config" / "arclink.env")


def _docker_host_repo_dir(cfg: Config) -> Path:
    configured = str(os.environ.get("ARCLINK_DOCKER_HOST_REPO_DIR") or "").strip()
    if configured:
...
def _docker_host_private_dir(cfg: Config, repo_dir: Path) -> Path:
    configured = str(os.environ.get("ARCLINK_DOCKER_HOST_PRIV_DIR") or "").strip()
    if configured:
        return Path(configured).resolve(strict=False)
    return repo_dir / "arclink-priv"
...
def _operator_child_env(cfg: Config) -> dict[str, str]:
    env = os.environ.copy()
    env["ARCLINK_CONFIG_FILE"] = _config_file_for_child_process(cfg)
    if _docker_mode():
        repo_dir = _docker_host_repo_dir(cfg)
        private_dir = _docker_host_private_dir(cfg, repo_dir)
        env["ARCLINK_DOCKER_MODE"] = "1"
        env["ARCLINK_CONTAINER_RUNTIME"] = "docker"
        env["ARCLINK_COMPONENT_UPGRADE_MODE"] = "docker"
        env["ARCLINK_REPO_DIR"] = str(repo_dir)
        env["ARCLINK_PRIV_DIR"] = str(private_dir)
        env["ARCLINK_PRIV_CONFIG_DIR"] = str(private_dir / "config")
        env["STATE_DIR"] = str(private_dir / "state")
        env["ARCLINK_CONFIG_FILE"] = str(private_dir / "config" / "docker.env")
    if not env.get("HOME"):
        try:
    ...
    if cfg.upstream_repo_url:
        env.setdefault("ARCLINK_UPSTREAM_REPO_URL", str(cfg.upstream_repo_url))
    if cfg.upstream_branch:
        env.setdefault("ARCLINK_UPSTREAM_BRANCH", str(cfg.upstream_branch))
    if cfg.upstream_deploy_key_enabled:
        env.setdefault("ARCLINK_UPSTREAM_DEPLOY_KEY_ENABLED", "1")
    if cfg.upstream_deploy_key_path:
        env.setdefault("ARCLINK_UPSTREAM_DEPLOY_KEY_PATH", str(cfg.upstream_deploy_key_path))
    if cfg.upstream_known_hosts_file:
        env.setdefault("ARCLINK_UPSTREAM_KNOWN_HOSTS_FILE", str(cfg.upstream_known_hosts_file))
    return env
...
def _run_configure_agent_backup(
    ...
    *,
    ...
    result = _run_as_user(
        ...
        cmd=args,
        extra={"ARCLINK_CONFIG_FILE": _config_file_for_child_process(cfg)},
    )
    log_path.parent.mkdir(parents=True, exist_ok=True)
    log_path.write_text(
        (result.stdout or "")
...
def _agent_backup_public_key(hermes_home: Path) -> str:
    pending = _read_shell_env_file(hermes_home / "state" / "arclink-agent-backup.pending.env")
    active = _read_shell_env_file(hermes_home / "state" / "arclink-agent-backup.env")
...
def _service_url_env(cfg: Config) -> dict[str, str]:
    return {
        "ARCLINK_MCP_URL": str(os.environ.get("ARCLINK_MCP_URL") or f"http://127.0.0.1:{cfg.public_mcp_port}/mcp"),
        "ARCLINK_BOOTSTRAP_URL": str(os.environ.get("ARCLINK_BOOTSTRAP_URL") or os.environ.get("ARCLINK_MCP_URL") or f"http://127.0.0.1:{cfg.public_mcp_port}/mcp"),
        "ARCLINK_QMD_URL": str(os.environ.get("ARCLINK_QMD_URL") or cfg.qmd_url),
    }
...
def _provision_user_access_surfaces(
    ...
    *,
    ...
    result = _run_as_user(
        ...
        extra={"ARCLINK_AGENT_VAULT_DIR": str(cfg.vault_dir)},
    )
    if result.returncode != 0:
        raise RuntimeError((result.stderr or result.stdout or "install-agent-user-services failed").strip())
    _assert_user_unit_active(
        ...
        hermes_home=hermes_home,
        uid=uid,
        unit_name="arclink-user-agent-dashboard.service",
    )
    _assert_user_unit_active(
        unix_user=unix_user,
        home=home,
...
def _shared_notion_self_serve_configured() -> bool:
    return bool(
        str(config_env_value("ARCLINK_SSOT_NOTION_TOKEN", "") or "").strip()
        and str(config_env_value("ARCLINK_SSOT_NOTION_SPACE_ID", "") or "").strip()
    )


def _agent_backup_onboarding_enabled() -> bool:
    return str(config_env_value("ARCLINK_AGENT_BACKUP_ONBOARDING_ENABLED", "1") or "1").strip().lower() not in {
        "0",
        "false",
        "no",
        "off",
...
def _stage_provider_secret_for_user(
    *,
    ...
    staged_path = secrets_dir / f"arclink-onboarding-{session_id}.secret"
...
def _seed_user_provider(cfg: Config, *, session: dict, unix_user: str, home: Path, hermes_home: Path, uid: int) -> dict[str, Any]:
    ...
    script_path = cfg.repo_dir / "python" / "arclink_headless_hermes_setup.py"
...
def _assert_user_gateway_active(*, unix_user: str, home: Path, hermes_home: Path, uid: int) -> None:
    ...
    while time.time() < deadline:
        result = _run_as_user(
            ...
            uid=uid,
            hermes_home=hermes_home,
            cmd=["systemctl", "--user", "is-active", "arclink-user-agent-gateway.service"],
        )
    ...
    *,
    ...
    if _docker_mode():
        ...
        result = _run_as_user(
            ...
            uid=uid,
            hermes_home=hermes_home,
            cmd=["systemctl", "--user", "start", "arclink-user-agent-refresh.service"],
    )
    if result.returncode != 0:
    ...
    state_path = hermes_home / "state" / "arclink-vault-reconciler.json"
...
def _restart_user_agent_gateway_if_enabled(
    *,
    ...
    enabled_result = _run_as_user(
        ...
        uid=uid,
        hermes_home=hermes_home,
        cmd=["systemctl", "--user", "is-enabled", "arclink-user-agent-gateway.service"],
    )
    ...
    restart_result = _run_as_user(
        ...
        uid=uid,
        hermes_home=hermes_home,
        cmd=["systemctl", "--user", "restart", "arclink-user-agent-gateway.service"],
    )
...
def _run_pending_onboarding_notion_verifications_locked(conn, cfg: Config) -> None:
    ...
    for session in list_onboarding_sessions(conn, redact_secrets=False):
        ...
        if status == "pending":
            try:
                page_payload = retrieve_notion_page(page_id=str(claim.get("notion_page_id") or ""), token=str(config_env_value("ARCLINK_SSOT_NOTION_TOKEN", "") or ""), api_version=str(config_env_value("ARCLINK_SSOT_NOTION_API_VERSION", "") or "") or "2026-03-11")
        ...
        _notify_user_via_curator(
            ...
            session=updated,
            message=(
                "Verified. I can now write to shared Notion on your behalf through ArcLink's brokered rail, "
...
def _run_pending_operator_actions(conn, cfg: Config) -> None:
    ...
    _queue_operator_message(
        ...
        cfg,
        (
            "Starting ArcLink upgrade from the operator action queue.\n"
    ...
    if result.returncode == 0:
        ...
        _queue_operator_message(
            ...
            cfg,
            (
                "ArcLink upgrade completed successfully.\n"
                f"Requested by: {requested_by}\n"
                f"Log: {log_path}"
            ),
        )
...
def _discord_agent_handoff_text(*, bot_label: str, confirmation_code: str) -> str:
    return (
        f"Your ArcLink lane is ready here with `{bot_label}`.\n\n"
        ...
        if result.returncode == 0:
            ...
            if session is not None and phase == "prepare":
                ...
                send_session_message(
                    cfg,
                    updated,
                    f"Private agent backup is active for `{owner_repo or remote}`. Sessions, soul, config, memories, skills/plugins, and selected ArcLink state will back up through Hermes cron every 4 hours.",
                )
        ...
        env = _user_subprocess_env(
            ...
            extra={
                **_service_url_env(cfg),
                ...
                "ARCLINK_INIT_SKIP_GATEWAY_SETUP": "1",
                "ARCLINK_INIT_MODEL_PRESET": model_preset,
                "ARCLINK_INIT_CHANNELS": ",".join(channels),
                "ARCLINK_SHARED_REPO_DIR": str(cfg.repo_dir),
                "RUNTIME_DIR": str(cfg.runtime_dir),
                "ARCLINK_ACTIVATION_TRIGGER_PATH": str(activation_path),
                "ARCLINK_AGENT_HERMES_HOME": str(hermes_home),
                "ARCLINK_AGENT_SERVICE_MANAGER": "docker-supervisor" if _docker_mode() else "systemd",
                "ARCLINK_DOCKER_MODE": "1" if _docker_mode() else "0",
                "ARCLINK_CONTAINER_RUNTIME": "docker" if _docker_mode() else os.environ.get("ARCLINK_CONTAINER_RUNTIME", ""),
            },
        )
