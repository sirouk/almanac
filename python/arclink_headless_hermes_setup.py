...
def _org_skill_library_bases(hermes_home: Path) -> list[Path]:
    ...
    for env_key in ("ARCLINK_AGENT_VAULT_DIR", "VAULT_DIR"):
        value = str(os.environ.get(env_key) or "").strip()
        if value:
    ...
    bases.extend(
        [
            home / "ArcLink" / "Agents_Skills",
            hermes_home / "ArcLink" / "Agents_Skills",
...
def _org_profile_soul_and_context(bot_name: str, unix_user: str, user_name: str = "") -> tuple[str | None, dict[str, Any] | None]:
    try:
        ...
        from arclink_org_profile import render_soul_for_identity
            ...
            Template(template).substitute(
                {
                    ...
                    "org_primary_project": _identity_value(
                        _config_value("ARCLINK_ORG_PRIMARY_PROJECT"),
                        "the work your user puts in front of you",
                    ),
                    "org_timezone": _identity_value(_config_value("ARCLINK_ORG_TIMEZONE", "Etc/UTC"), "Etc/UTC"),
...
def _seed_arclink_identity(bot_name: str, unix_user: str, user_name: str = "", agent_title: str = "") -> dict[str, str]:
    ...
    prefill_path = state_dir / "arclink-prefill-messages.json"
    ...
    if org_profile_context:
        try:
            from arclink_org_profile import identity_values_from_context
    ...
    prefill_messages = [
        {
            ...
            "content": (
                ...
                "as active defaults, not passive extras. Use qmd and the wired ArcLink "
                ...
                "installed skills, plugin-managed context, and ArcLink-provisioned rails first. Treat "
                ...
                "them in your own local memory entries rather than rewriting plugin-managed context. The "
                ...
                "channels, and ArcLink resources; never browse other users' home directories or "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Seed a Hermes home for headless ArcLink onboarding.")
    ...
    parser.add_argument(
        "--identity-only",
        action="store_true",
        help="Only refresh the ArcLink SOUL.md and prefill config.",
    )
