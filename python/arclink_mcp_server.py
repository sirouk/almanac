...
from arclink_control import (
    ...
    backend_client_allowed as control_backend_client_allowed,
    bootstrap_status,
...
TOOLS = {
    ...
    "vault.search": "Fast search of shared/private vault knowledge through ArcLink's qmd-backed vault rail. Prefer vault.search-and-fetch when you need the body to answer, especially for PDFs.",
    ...
    "notion.search": "Search shared Notion knowledge through ArcLink's qmd-backed indexed Notion rail. Call when the user wants a phrase/title discovery only; prefer notion.search-and-fetch when you also need the body to answer.",
    ...
    "notion.query": "Run a live structured query against one shared Notion database/data source. Prefer when the user gives an exact database/data-source target or explicitly asks for a live structured refresh. For broad plate/focus/task orientation, start from [managed:today-plate] and qmd-backed knowledge.search-and-fetch; do not fan out live queries across discovered databases in a chat turn. For broad knowledge lookup by phrase, call notion.search-and-fetch.",
    ...
    "knowledge.search": "Search ArcLink knowledge across both vault/PDF and shared Notion rails when the source is unclear. Bounded source-agnostic discovery; prefer precise vault.*, notion.*, or ssot.* tools when the user clearly names the lane.",
    "knowledge.search-and-fetch": "Search and fetch ArcLink knowledge across vault/PDF and shared Notion rails in one bounded call. Best first move for broad user questions that could live in files, PDFs, cloned docs, or shared Notion pages.",
}
...
def _ensure_mcp_session(method: str, session_id: str | None, sessions: set[str]) -> tuple[str | None, bool]:
    ...
    Hermes chat sessions can outlive an ArcLink MCP service repair or upgrade.
...
AGENT_TOKEN_PROP = {
    "type": "string",
    "minLength": 1,
    "description": "Harness-injected agent bootstrap token. Hermes agents should omit this field; arclink-managed-context fills it before dispatch. Legacy/operator CLIs may still pass it explicitly.",
}
...
SSOT_PAYLOAD_PROP = {
    "type": "object",
    "description": "Notion write payload. update/insert usually use properties. For create_page, pass title/name plus optional children and omit parent. For create_database, pass title plus properties or initial_data_source.properties and omit parent. For append, pass exactly {'children': [...]} with no 'after'. create_page/append children are capped at 100 blocks per call. ArcLink strips/stamps Changed By, Author, and Requested By attribution fields.",
    "additionalProperties": True,
}
...
TOOL_SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "bootstrap.request": _schema(
        {
            ...
            "model_preset": {"type": "string", "description": "Optional requested ArcLink model preset."},
    ...
    "academy.propose-resource": _schema(
        {
            ...
            "proposal_kind": {"type": "string", "enum": ["add_resource", "discontinue_resource"], "default": "add_resource", "description": "Use add_resource for a candidate source or discontinue_resource to flag a dead-end central source for Trainer/PG-PROVIDER review."},
            "target_source_uid": {"type": "string", "description": "Optional central Academy source_uid when proposal_kind is discontinue_resource."},
            ...
            "summary": {"type": "string", "description": "Compressed derived notes, or the concise discontinuation reason. Do not submit raw article bodies, transcripts, secrets, or paid material."},
        ...
        required=("lane_id", "title", "summary"),
    ),
    ...
    "ssot.preflight": _schema(
        {
            "token": AGENT_TOKEN_PROP,
            "operation": {"type": "string", "enum": ["insert", "update", "append", "create_page", "create_database"], "description": "Mutating SSOT operation to check without applying."},
    ...
    "ssot.write": _schema(
        {
            "token": AGENT_TOKEN_PROP,
            "operation": {"type": "string", "enum": ["insert", "update", "append", "create_page", "create_database"], "description": "Mutating SSOT operation. Archive/delete/trash/destroy are intentionally unsupported and rejected by the broker."},
            "target_id": {"type": "string", "description": "Required for append/update. For insert, use the parent page/database/data-source id or URL when the configured SSOT target is not enough. For create_page/create_database, use the shared parent page id/URL or omit to use the configured ArcLink root page."},
            "payload": SSOT_PAYLOAD_PROP,
    ...
    "knowledge.search": _schema(
        {
            "token": AGENT_TOKEN_PROP,
            "query": {"type": "string", "minLength": 1, "description": "Search text across ArcLink vault/PDF and shared Notion knowledge."},
            "sources": {
                "type": "array",
    ...
    "knowledge.search-and-fetch": _schema(
        {
            "token": AGENT_TOKEN_PROP,
            "query": {"type": "string", "minLength": 1, "description": "Search and fetch text across ArcLink vault/PDF and shared Notion knowledge."},
            "sources": {
                "type": "array",
                "items": {"type": "string", "enum": ["vault", "notion"]},
                "description": "Optional rails to search. Defaults to both ['vault', 'notion'].",
...
def _qmd_query_arguments(arguments: dict, *, limit_key: str = "limit", include_vec: bool = True) -> dict[str, Any]:
    ...
    intent = str(arguments.get("intent") or "").strip() or f"Answer from ArcLink vault knowledge: {query}"
...
def _split_markdown_metadata_block(text: str) -> tuple[str, bool, str]:
    ...
    if end < 0:
        return text, False, ""
    metadata = text[len("---\n") : end].strip()
    return text[end + len(marker) :].lstrip(), True, metadata
...
def _markdown_file_metadata(path: Path) -> dict[str, str]:
    if not path.is_file():
        return {}
    try:
        lines = path.read_text(encoding="utf-8", errors="replace").splitlines()
...
def _case_preserving_child(parent: Path, name: str) -> Path:
    ...
    for child in children:
        if child.name.casefold() == folded:
            return child
    loose = _loose_path_key(name)
    if loose:
        for child in children:
            if _loose_path_key(child.name) == loose:
                return child
    return candidate


def _case_preserving_vault_path(root: Path, rel_path: str) -> tuple[Path, str]:
    current = root
    parts = [part for part in Path(str(rel_path or "")).parts if part not in ("", ".")]
    resolved_parts: list[str] = []
    for part in parts:
        current = _case_preserving_child(current, part)
        resolved_parts.append(current.name)
    return current.resolve(), "/".join(resolved_parts)
...
def _create_agent_share_request(conn: sqlite3.Connection, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "copy_duplicate_policy": _LINKED_COPY_DUPLICATE_POLICY,
        "copy_duplicate_destination_roots": list(_LINKED_COPY_DUPLICATE_DESTINATION_ROOTS),
        "copy_duplicate_policy_detail": _LINKED_COPY_DUPLICATE_POLICY_DETAIL,
        **result.payload,
    }
...
def _vault_source_path_for_qmd_ref(cfg: Config, source_ref: str) -> tuple[str, str, Path | None, dict[str, str]]:
    ...
    if collection == "vault":
        source_path, resolved_rel_path = _case_preserving_vault_path(cfg.vault_dir, rel_path)
        return collection, resolved_rel_path or rel_path, source_path, {}
    if collection == "vault-pdf-ingest":
        markdown_root = cfg.state_dir / "pdf-ingest" / "markdown"
        generated_path, generated_rel_path = _case_preserving_vault_path(markdown_root, rel_path)
        file_metadata = _markdown_file_metadata(generated_path)
...
def _vault_source_metadata(
    ...
    *,
    ...
    if generated_metadata:
        ...
        metadata["source_type"] = str(generated_metadata.get("arclink_source_type") or "generated")
    else:
...
def _extract_qmd_text_result(result: dict[str, Any], *, body_char_limit: int) -> dict[str, Any]:
    ...
    _body_text, metadata_present, metadata = _split_markdown_metadata_block(raw_text)
    text, truncated = _trim_text(raw_text, body_char_limit)
    metadata_notice = (
        "Leading Markdown YAML metadata is included at the top of text and duplicated into metadata; "
        "inspect it before judging metadata-driven files such as SKILL.md."
        if metadata_present
        else ""
    )
    return {
        "ok": bool(text or metadata),
        "uri": uri,
        "mime_type": mime_type,
        "text": text,
        "text_truncated": truncated,
        "metadata_present": metadata_present,
        "metadata_format": "yaml" if metadata_present else "",
        "metadata": metadata,
        # Compatibility aliases for older agent recipes and clients.
        "frontmatter_present": metadata_present,
...
def _notion_index_hit_file(hit: dict[str, Any]) -> str:
    for key in ("file", "path"):
        value = str(hit.get(key) or "").strip()
        if value:
            return value
    raw = hit.get("raw_result")
...
def _qmd_fetch_notion_index_file(cfg: Config, arguments: dict[str, Any], *, file_value: str) -> dict[str, Any]:
    body_char_limit = _clamp_int(
        arguments.get("body_char_limit"),
        default=4000,
        minimum=200,
    ...
    return {
        ...
        "target_kind": "indexed-markdown",
        "indexed": True,
        "file": fetch_args["file"],
        "uri": str(compact.get("uri") or ""),
        "markdown": str(compact.get("text") or ""),
        "markdown_truncated": bool(compact.get("text_truncated")),
        "metadata_present": bool(compact.get("metadata_present")),
        "metadata_format": str(compact.get("metadata_format") or ""),
        "metadata": str(compact.get("metadata") or ""),
        "frontmatter_present": bool(compact.get("frontmatter_present")),
        "frontmatter": str(compact.get("frontmatter") or ""),
        "metadata_stripped": bool(compact.get("metadata_stripped")),
        "stripped_metadata": str(compact.get("stripped_metadata") or ""),
        "metadata_notice": str(compact.get("metadata_notice") or ""),
        "fromLine": fetch_args["fromLine"],
        "maxLines": fetch_args["maxLines"],
        "lineNumbers": fetch_args["lineNumbers"],
        "body_char_limit": body_char_limit,
        "fetch_source": "qmd-index-fallback",
    }
...
def _normalize_ssot_write_result(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(notion_result, dict):
        ...
        notion_object = str(notion_result.get("object") or "").strip().lower()
        if notion_url:
            result.setdefault("url", notion_url)
            if notion_object == "database":
                result.setdefault("database_url", notion_url)
            else:
                result.setdefault("page_url", notion_url)
        if notion_id:
            result.setdefault("result_id", notion_id)
            if notion_object == "database":
                result.setdefault("database_id", notion_id)
    return result


def backend_client_allowed(remote_ip: str) -> bool:
    return control_backend_client_allowed(remote_ip)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the ArcLink control-plane HTTP server.")
    parser.add_argument("--host", default=None)
    parser.add_argument("--port", type=int, default=None)
    return parser.parse_args()


class ArcLinkServer(ThreadingHTTPServer):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _rpc_error(
        ...
        self.send_header("Content-Type", "application/json")
        self.send_header("Content-Length", str(len(raw)))
        self.send_header("X-ArcLink-MCP-Error-Status", str(status))
    ...
    def _request_source_ip(self, arguments: dict) -> str:
        ...
        if (
            ...
            and declared_ip
            and os.environ.get("ARCLINK_ALLOW_LOOPBACK_SOURCE_IP_OVERRIDE", "0") == "1"
        ):
            return declared_ip
        return remote_ip
    ...
    def _tailscale_identity(self) -> dict[str, str]:
        ...
        """
        if os.environ.get("ARCLINK_TRUST_TAILSCALE_PROXY_HEADERS", "0") != "1":
            return {}
        ...
        with connect_db(cfg) as conn:
            ...
            if tool_name == "vault.search-and-fetch":
                ...
                try:
                    ...
                    # like "Example Lattice" and beats falling back to filesystem
            ...
            if tool_name == "academy.propose-resource":
                ...
                proposal = record_academy_resource_proposal(
                    ...
                    proposal_kind=str(arguments.get("proposal_kind") or "add_resource"),
                    target_source_uid=str(arguments.get("target_source_uid") or ""),
                ...
                for hit in raw_hits:
                    ...
                    if target_id:
                        try:
                            ...
                            fetched_entry["fetch"] = _compact_notion_fetch_result(
                                fetch_result,
                                body_char_limit=body_char_limit,
                            )
                        except Exception as exc:  # noqa: BLE001
...
def main() -> None:
    ...
    server = ArcLinkServer((host, port), Handler, cfg)
    try:
