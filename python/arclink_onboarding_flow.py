...
from arclink_model_providers import provider_default_model, provider_recommended_models
from arclink_onboarding_completion import remote_hermes_wrapper_name
...
def shared_notion_home_url() -> str:
    root_url = config_env_value("ARCLINK_SSOT_NOTION_ROOT_PAGE_URL", "").strip()
    if root_url:
        return root_url
    return config_env_value("ARCLINK_SSOT_NOTION_SPACE_URL", "").strip()
...
def _org_profile_match_candidates(conn, cfg: Config, incoming: IncomingMessage, full_name: str) -> list[dict[str, Any]]:
    try:
        from arclink_org_profile import load_applied_profile
    except Exception:
        return []
...
def _org_provider_secret() -> str:
    return config_env_value("ARCLINK_ORG_PROVIDER_SECRET", "").strip()
...
def _org_provider_reasoning_effort(cfg: Config | None = None) -> str:
    ...
    configured = configured or config_env_value("ARCLINK_ORG_PROVIDER_REASONING_EFFORT", "medium")
...
def _parse_github_owner_repo(raw_text: str) -> tuple[str, str]:
    ...
    if not GITHUB_OWNER_REPO_PATTERN.fullmatch(value):
        return "", "Reply with a GitHub `owner/repo` path like `example/arclink-guide`, or `skip`."
...
def _operator_review_message(cfg: Config, session: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Approve: ./bin/arclink-ctl onboarding approve {session_id}",
        f"Deny: ./bin/arclink-ctl onboarding deny {session_id} --reason 'optional reason'",
    ]
...
def _org_provider_selection_note(cfg: Config, session: dict[str, Any]) -> str:
    ...
    wrapper_name = remote_hermes_wrapper_name(
        ...
        org_name=config_env_value("ARCLINK_ORG_NAME", "").strip() or config_env_value("ARCLINK_NAME", "arclink").strip(),
    )
...
def _shared_tailnet_host() -> str:
    try:
        from arclink_resource_map import shared_tailnet_host
...
def session_prompt(cfg: Config, session: dict[str, Any]) -> str:
    ...
    if state == "awaiting-name":
        return (
            "Hi, I’m ArcLink’s Curator. I’ll help set up your private agent lane one step at a time.\n\n"
    ...
    if state == "awaiting-unix-user":
        ...
        return (
            ...
            "ArcLink creates a private Unix account for your agent lane. That gives you a home directory, Hermes state, and code workspace on the shared host.\n\n"
    ...
    if state == "awaiting-agent-backup-repo":
        ...
        example = f"{unix_user}/arclink-{bot_slug}"
        return (
    ...
    if state == "awaiting-notion-access":
        ...
        lines = [
            ...
            "",
            "First, make sure you can open the shared ArcLink page:",
        ]
        if shared_page_url:
    ...
    if state == "completed":
        ...
        if bot_platform == "discord" and bot_username:
            return (
                ...
                "It already has the ArcLink skills active by default, plus the shared Vault/qmd wiring. "
        ...
        return "Your agent lane is live. It already has the ArcLink skills active by default, plus the shared Vault/qmd wiring."
...
def _completion_reply_for_session(
    ...
    *,
    ...
    from arclink_onboarding_completion import completion_bundle_for_session
    ...
    with connect_db(cfg) as conn:
        ...
        if state == "awaiting-notion-verification":
            if lower == "skip":
                ...
                if claim_id:
                    try:
                        from arclink_control import mark_notion_identity_claim
