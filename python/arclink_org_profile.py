    ...
    if previous == content:
        if mode is not None:
            try:
                ...
                pass
        return False
    fd, tmp_path = tempfile.mkstemp(dir=str(path.parent), prefix=".arclink-org-profile-", suffix=".tmp")
    try:
...
def _semantic_report(profile: dict[str, Any], cfg: Any | None = None) -> tuple[list[str], list[str]]:
    ...
    for person in people:
        ...
        if not serves:
            warnings.append(f"people.{person_id}.agent.serves is empty; ArcLink will treat it as serving {person_id}")
...
def _display_source_path(source_path: Path) -> str:
    ...
    if "arclink-priv" in parts:
        index = parts.index("arclink-priv")
...
def render_soul_overlay(context: dict[str, Any]) -> str:
    ...
    lines = [
        BEGIN_SOUL_MARKER,
        "ArcLink operating-profile overlay:",
...
def materialize_agent_context(hermes_home: Path, context: dict[str, Any]) -> dict[str, Any]:
    state_path = hermes_home / "state" / "arclink-org-profile-context.json"
