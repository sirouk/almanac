...
from arclink_org_profile import format_preview, load_profile, preview_payload, validate_profile


REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_SCHEMA_URL = "https://arclink.local/schema/org-profile.schema.json"
...
def default_profile_path() -> Path:
    try:
        ...
        from arclink_org_profile import default_profile_path as configured_profile_path

        cfg = Config.from_env()
        return configured_profile_path(cfg)
    except Exception:
        return REPO_ROOT / "arclink-priv" / "config" / PROFILE_FILENAME
...
def profile_starter() -> dict[str, Any]:
    return {
        ...
        "organization": {
            ...
            "name": "My ArcLink",
            "profile_kind": "hybrid",
            "scope": "Solo, family, project, team, or organization context managed by ArcLink.",
        ...
        "roles": {
            "operator": {
                "description": "Owns ArcLink deployment, profile policy, access, and production alignment.",
                "responsibilities": ["Maintain the private operating profile and approve high-impact changes."],
                "decision_authority": ["Final approval for profile application and privileged ArcLink changes."],
        ...
        "people": [
            {
                ...
                "human_accountability": {
                    "accountable_for": ["Operating ArcLink safely."],
        ...
        "teams": [
            {
                ...
                "responsibilities": ["Maintain ArcLink's profile, deployment, and safety rails."],
            }
        ],
        "relationships": [],
        "agent_lineage": {
...
def edit_organization(profile: dict[str, Any]) -> None:
    ...
    name = prompt_line("Operating context name", str(org.get("name") or "My ArcLink"), required=True)
    org["name"] = name
    ...
    org["scope"] = prompt_multiline("Scope / what ArcLink is being aligned around", str(org.get("scope") or ""))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Interactively build or edit an ArcLink private operating profile.")
    parser.add_argument("--file", default="", help="Profile YAML path. Defaults to arclink-priv/config/org-profile.yaml.")
