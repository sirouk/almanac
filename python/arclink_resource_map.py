...
def shared_resource_lines(
    *,
    ...
    public_mcp_port: int,
    qmd_path: str = "/mcp",
    arclink_mcp_path: str = "/arclink-mcp",
    extra_mcp_label: str = "External knowledge rail",
    extra_mcp_url: str = "",
    ...
    if host:
        ...
        lines.append(f"ArcLink MCP control rail: http://{public_mcp_host}:{public_mcp_port}/mcp")

    if extra_mcp_url:
        label = str(extra_mcp_label or "").strip() or "External knowledge rail"
        lines.append(f"{label}: {extra_mcp_url}")
...
def managed_resource_lines(
    *,
    ...
    if workspace_root_text:
        ...
        lines.append(f"ArcLink vault: {Path(workspace_root_text) / 'ArcLink'}")
