# Almanac Architecture Map For ArcLink Transformation

Source: `sirouk/almanac` cloned into this workspace on 2026-05-01. Current checkout head at prep time: `b91e9a9 Harden live agent smoke timeout handling`.

## Core Mental Model

Almanac is a shared-host operating layer for Hermes agents. The current product shape is operator-led: one Curator agent owns onboarding, operator approval, notifications, upgrades, and user lane repair. Each enrolled user receives an isolated Hermes lane backed by a host Unix user or a Docker-supervised equivalent. The system is deliberately not a chatbot wrapper; it is a control plane for durable agent identity, context, file memory, Notion rails, provider auth, bot gateways, health, backup, and access surfaces.
...
Almanac already has two supported paths.
...
- `nextcloud`: shared vault UI mounted from `almanac-priv/vault`.
- `almanac-mcp`: control-plane MCP server over HTTP.
...
Current deployed layout is rooted around `almanac-priv/`:

- `config/docker.env` or `config/almanac.env`: runtime configuration and generated secrets.
- `vault/`: durable shared file memory.
- `state/almanac-control.sqlite3`: SQLite control-plane DB.
...
`python/almanac_control.py` owns the SQLite schema and control-plane logic. Important existing tables:
...
`python/almanac_onboarding_flow.py` implements the Curator onboarding state machine. Notable states:
...
`python/almanac_enrollment_provisioner.py` drives provisioning.
...
8. `_seed_user_provider` runs `almanac_headless_hermes_setup.py` with staged provider credentials.
...
10. `_refresh_user_agent_memory` materializes `HERMES_HOME/state/almanac-vault-reconciler.json` for the managed-context plugin.
...
`python/almanac_agent_access.py` and `bin/run-agent-code-server.sh` create access state and run per-agent web surfaces.
...
- Nextcloud access synced through `almanac_nextcloud_access.py`.
...
The `plugins/hermes-agent/almanac-managed-context` plugin is central and should be preserved/rebranded carefully.

It:

- Reads `HERMES_HOME/state/almanac-vault-reconciler.json`.
...
- Emits JSONL telemetry at `HERMES_HOME/state/almanac-context-telemetry.jsonl`.

Memory synthesis lives in `python/almanac_memory_synthesizer.py`, with cards stored in `memory_synthesis_cards`. It scans vault and Notion sources, hashes source signatures, calls an OpenAI-compatible model, normalizes cards, and writes status.
...
`python/almanac_onboarding_provider_auth.py` supports provider setup, Chutes thinking mode handling, Codex device auth, Anthropic PKCE, and API key normalization.
...
- Legacy `ALMANAC_*` env vars may need compatibility aliases during migration, but new public docs/config should prefer `ARCLINK_*`.
- Python module/script names can be migrated gradually if tests and imports are updated atomically. Avoid halfway states where UI says ArcLink but service files, messages, and dashboard still say Almanac.
...
Do not throw away Almanac. ArcLink is Almanac sharpened into a self-serve SaaS deployment product with first-class user and admin dashboards, payment/DNS orchestration, Chutes-first inference, and scalable operations.
