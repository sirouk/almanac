...
| Raven chat scope | `python/arclink_public_bots.py`, `python/arclink_notification_delivery.py`, `python/arclink_telegram.py`, `python/arclink_discord.py`, Raven docs/tests | Current truth is Raven-owned slash-command control plus selected-agent bridging for onboarded users' normal messages and non-Raven slash commands; `notification-delivery` executes the selected deployment's Hermes gateway container and returns the agent reply to the same linked channel. |
