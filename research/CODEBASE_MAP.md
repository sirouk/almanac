...
| Raven chat scope | `python/arclink_public_bots.py`, `python/arclink_public_bot_commands.py`, `python/arclink_notification_delivery.py`, `python/arclink_telegram.py`, `python/arclink_discord.py`, Raven docs/tests | Current truth is `/raven`-namespaced Raven control in active Telegram chats plus selected-agent bridging for onboarded users' normal messages and active-agent slash commands; command registration refreshes active Telegram per-chat scopes and alerts on collisions, while `notification-delivery` executes the selected deployment's Hermes gateway container and returns the agent reply to the same linked channel. |
