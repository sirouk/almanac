...
| Raven is a truthful post-onboarding control conduit | Agent inventory, switching, labels, explicit channel identifiers, channel linking aliases, share approvals, upgrade guidance, local Raven display-name preferences, and selected-agent freeform bridging have local coverage | Preserve channel/account Raven display-name scoping, selected-agent labels, slash-command ownership, queued freeform agent turns, and platform-profile truthfulness | Public bot, notification delivery, Telegram, Discord tests |
...
| Raven chat-scope decision is explicit | Current local truth is Raven as the public control conduit for slash commands and the selected-agent bridge for onboarded-user freeform messages | Preserve freeform proxying only after account/deployment resolution, keep slash commands on Raven, and keep cross-user active-deployment checks | Public bot, notification delivery, Telegram, Discord tests |
