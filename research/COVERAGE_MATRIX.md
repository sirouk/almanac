...
| Raven chat-scope decision is explicit | Current local truth is Raven as the public control conduit for Raven-owned slash commands and the selected-agent bridge for onboarded-user normal messages plus non-Raven slash commands | Preserve proxying only after account/deployment resolution, keep Raven controls on Raven, and keep cross-user active-deployment checks | Public bot, notification delivery, Telegram, Discord tests |
