...
- Repo-wide ruff/pyflakes still fail on pre-existing non-ArcLink-slice issues outside this lint gate's scoped surface: unused imports/variables in `bin/notion-page-pdf-export.py`, `python/almanac_ctl.py`, `python/almanac_enrollment_provisioner.py`, `python/almanac_headless_hermes_setup.py`, `python/almanac_health_watch.py`, `python/almanac_mcp_server.py`, and related tests, plus import-order findings in memory synthesizer tests/modules.
