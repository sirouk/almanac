...
declare -A ITEMS=(
  ...
  [W6]="CANON-02,CANON-03|D2,D1|hosted-api-transport,web-product-surface|W2 ALREADY moved _remote_ip_from_headers onto ARCLINK_TRUSTED_PROXY_CIDRS (do NOT re-do the resolver). W6 = (a) CANON-03 D1: ensure the ADMIN-allow CIDR set (_backend_client_allowed / admin gating) is DISTINCT from the proxy-trust set — admin authorization must NOT widen because a CIDR is a trusted proxy, and vice-versa; document the two roles; (b) the F1=A SELF-HEAL: auto-render ARCLINK_TRUSTED_PROXY_CIDRS to the internal bridge range in the CANONICAL lanes (compose.yaml + bin/docker-entrypoint.sh + bin/deploy.sh) so blessed proxied deployments keep resolving XFF; (c) verify the split end-to-end with regression tests (trusted-proxy != admin-allowed; unset proxy still fails closed; canonical lane renders the range).|F1=A: when ARCLINK_TRUSTED_PROXY_CIDRS is unset the resolver already fails closed (trust no proxy); the canonical lanes MUST now auto-render the bridge range so blessed paths self-heal. No migration shim."
