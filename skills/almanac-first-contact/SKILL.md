...
description: Use after Almanac enrolls a user agent to verify the default MCPs, install the default Almanac skills, resolve default vault subscriptions, and complete first-contact orientation.
---
...
The script expects these environment variables (all normally exported by `init.sh agent`):
...
- emit a curator brief-fanout ping so the Curator knows to push initial briefs
