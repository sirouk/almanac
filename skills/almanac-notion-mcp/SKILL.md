...
description: Optional personal/operator helper for cases where a user's own Notion workspace is already connected through the official Notion MCP; not part of Almanac's default shared Notion contract.
---
...
This is optional personal/operator tooling. It is not the core Almanac-wide
Notion retrieval path for every enrolled agent.

This skill is for the user's own Notion workspace reached through Notion's
hosted MCP, not the shared Almanac SSOT and not the default shared Almanac
...
2. use `almanac-ssot` / `ssot.read` when the task is governed shared
...
For a question like "what is Chutes Unicorn?":

1. search for `Chutes Unicorn`
