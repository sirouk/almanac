...
DEFAULT_REMOTE_SETUP_URL = "https://raw.githubusercontent.com/example/almanac/main/bin/setup-remote-hermes-client.sh"
...
def vault_root(home: str) -> str:
    return f"{home.rstrip('/')}/Almanac" if home else "~/Almanac"
...
def remote_lines(access: dict[str, object], identity: dict[str, object]) -> list[str]:
    ...
    setup_url = clean(access.get("remote_setup_url") or os.environ.get("ALMANAC_REMOTE_SETUP_URL") or DEFAULT_REMOTE_SETUP_URL)
...
access = load_json(state_dir / "almanac-web-access.json")
managed = load_json(state_dir / "almanac-vault-reconciler.json")
identity = load_json(state_dir / "almanac-identity-context.json")
home = home_root(access)
vault = vault_root(home)

lines = ["Almanac resources:", "", "Web access:"]
...
lines.append(f"- Almanac vault: {vault}")
lines.extend(
    [
        "",
        "Host helper:",
