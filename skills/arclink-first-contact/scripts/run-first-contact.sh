...
: "${ARCLINK_MCP_URL:?ARCLINK_MCP_URL is required}"
: "${ARCLINK_QMD_URL:?ARCLINK_QMD_URL is required}"
: "${ARCLINK_BOOTSTRAP_TOKEN_FILE:?ARCLINK_BOOTSTRAP_TOKEN_FILE is required}"
: "${ARCLINK_SHARED_REPO_DIR:?ARCLINK_SHARED_REPO_DIR is required}"
...
ARCLINK_AGENTS_STATE_DIR="${ARCLINK_AGENTS_STATE_DIR:-$ARCLINK_SHARED_REPO_DIR/arclink-priv/state/agents}"

if [[ ! -r "$ARCLINK_BOOTSTRAP_TOKEN_FILE" ]]; then
  echo "first-contact: cannot read bootstrap token at $ARCLINK_BOOTSTRAP_TOKEN_FILE" >&2
  exit 2
fi
...
python3 - "$ARCLINK_QMD_URL" "$qmd_probe_file" <<'PY'
...
session_id, init_payload = post(
    {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "arclink-first-contact", "version": "1.0"},
        },
    }
)
...
_, query_payload = post(
    {
        ...
        "params": {
            "name": "query",
            "arguments": {
                "searches": [{"type": "lex", "query": "ArcLink"}],
                "collections": ["vault"],
                "intent": "Verify qmd retrieval rail during ArcLink first-contact",
                "rerank": False,
                "limit": 1,
            },
        },
...
notion_expected="0"
if [[ -n "${ARCLINK_NOTION_INDEX_ROOTS:-}" || -n "${ARCLINK_SSOT_NOTION_SPACE_URL:-}" || -n "${ARCLINK_SSOT_NOTION_TOKEN:-}" ]]; then
  notion_expected="1"
fi
...
ARCLINK_MANAGED_PAYLOAD="$managed_file" ARCLINK_HERMES_HOME="$HERMES_HOME" \
PYTHONPATH="$ARCLINK_SHARED_REPO_DIR/python${PYTHONPATH:+:$PYTHONPATH}" \
python3 - <<'PY' >/dev/null
import json
import os
from pathlib import Path
...
arclink_control.write_managed_memory_stubs(hermes_home=hermes_home, payload=payload)
PY

python3 - "$catalog_file" "$refresh_file" "$status_file" "$managed_file" "$qmd_probe_file" "$notion_probe_file" "$ARCLINK_MCP_URL" "$ARCLINK_QMD_URL" "$HERMES_HOME" <<'PY'
import json
import sys
from pathlib import Path
...
state_path = Path(hermes_home) / "state" / "arclink-vault-reconciler.json"
legacy_stub_path = Path(hermes_home) / "memories" / "arclink-managed-stubs.md"
