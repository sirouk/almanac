---
name: arclink-ssot-connect
description: Optional helper for linking a user's separate personal Notion workspace through the official Notion MCP flow; not part of ArcLink's default shared Notion contract.
---
...
This is distinct from the shared ArcLink SSOT:

- `arclink-ssot` is for the operator-managed organizational Notion space
  brokered through ArcLink's own `ssot.read` / `ssot.write` rails
- `arclink-ssot-connect` is for the user's own Notion MCP connection
...
setup, not the default shared ArcLink Notion knowledge rail.

## Default Flow

1. clarify whether they mean the shared organizational SSOT or their own Notion
2. if they mean the shared SSOT, hand back to `arclink-ssot` or the operator
...
5. once connected, hand ongoing work to `arclink-notion-mcp`
...
If the user is on a shared-host ArcLink agent, keep the language simple:
...
- do not store user-owned Notion credentials in ArcLink plugin-managed context
...
- if the user wants organization-wide updates in the shared SSOT, use
  `arclink-ssot`, not their private Notion MCP
