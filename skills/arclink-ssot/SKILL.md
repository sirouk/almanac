...
that ArcLink agents converge around.

The SSOT in ArcLink is a shared Notion workspace. Curator and the enrolled
...
Preferred agent path: call the `arclink-mcp` MCP tools directly. Do not inspect
...
- For shared pages, lists, task tables, or org-wide databases, call
  ...
  `operation:"create_database"` so ArcLink creates it under the shared root
...
- when the shared database exposes `Changed By`, ArcLink stamps the verified
...
- you are operating inside a Curator-managed ArcLink deployment shared by the organization
...
- with the verified webhook live, treat shared Notion changes as minutes-scale after ArcLink batches and de-duplicates the event
- without the verified webhook, tolerate delayed updates by using the 1-hour Curator full-sweep fallback (configurable via ARCLINK_NOTION_INDEX_FULL_SWEEP_INTERVAL_SECONDS)
- expect ArcLink to batch and de-duplicate webhook-driven refresh work
...
- if the user explicitly wants wide retrieval across a separately connected
  ...
  `arclink-notion-mcp`; do not present that lane as the default shared ArcLink
...
- do not claim that native Notion history will show the human directly; it
  shows the ArcLink integration while the row-level attribution lives in
  `Changed By` plus ArcLink's local audit
- do not inspect other users' home directories or central deployment config to
  infer ownership; use the SSOT metadata and ArcLink rails already exposed to
  this agent
- if a requested action would archive or delete content, stop and ask first
- if the requested change appears to touch another user's records, ask for approval instead of guessing; ArcLink can now queue that approval instead of dropping the request
