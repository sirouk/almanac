---
name: arclink-vault-reconciler
description: Use when an agent needs to keep compact plugin-managed context aligned with an ArcLink vault behind qmd, run an initial reconciliation, or explain/repair the first-flight + refresh + Curator-triggered sync rails.
---
...
If the user is asking a normal vault-backed knowledge question rather than diagnosing memory drift, use `arclink-qmd-mcp` first instead of searching the repo or walking the sync rails.
...
The vault layout is allowed to be organization-shaped. Seeded folders such as
`Projects/` and `Repos/` are conventions, not required retrieval boundaries;
plain folders remain searchable through qmd, while `.vault` files define only
subscription and notification lanes.
...
## Existing ArcLink sync rails

Do not invent a second state machine when ArcLink already has the right rails.

There are three intended triggers:

1. first flight
   - `activate-agent.sh` runs `arclink-first-contact`
...
2. scheduled refresh
   - `arclink-user-agent-refresh.timer` runs every 4 hours
...
3. Curator-driven sync
   ...
   - the activation-trigger rail can cause `arclink-user-agent-refresh.service` to run immediately for affected agents
...
- `$HERMES_HOME/state/arclink-recent-events.json`

The shared control plane may also publish:

- `$ARCLINK_AGENTS_STATE_DIR/<agent-id>/managed-memory.json`
...
- which ArcLink skills are active defaults
...
When the user says memory is stale or asks how ArcLink sync works, do this in order:
...
6. report drift and repair the ArcLink-side rail, not Hermes internals

## Warm reload reality

Today, ArcLink refreshes plugin-managed context state immediately, and the preferred no-core-code path is to let the shipped `arclink-managed-context` Hermes plugin inject refreshed local ArcLink context into future turns.

Important distinctions:

- `user-agent-refresh.sh` rewrites the local plugin state correctly
- Hermes built-in `MEMORY.md` is long-lived user memory, not ArcLink's hot-swap rail
- the `arclink-managed-context` plugin injects ephemeral ArcLink context through Hermes's plugin hook system on future turns

So if the user asks why the newest managed stub text is not visible in the built-in memory block of an already-active chat, do not misdiagnose the ArcLink rail as broken.

The right interpretation is usually:

- disk refresh succeeded
- built-in `MEMORY.md` was not the source of the dynamic context
- the plugin-injected ArcLink context is the warm path for future turns without touching Hermes core
...
1. ArcLink publishes a stable `managed_memory_revision` with the canonical payload
...
3. the shipped `arclink-managed-context` plugin reads that local state on `pre_llm_call`
4. the plugin injects compact refreshed ArcLink context into the current user turn when:
   - the session is new,
   - the managed revision changed,
   - or the user asks an ArcLink-relevant question
...
With the shipped plugin path, ArcLink does not need Hermes core edits to achieve practical next-turn warm reload.

So when investigating this class of issue:

- repair ArcLink first if the payload, trigger, timer, or local state file writes are wrong
- verify the plugin is installed under `HERMES_HOME/plugins/arclink-managed-context`
- verify the plugin's local state source under `HERMES_HOME/state/arclink-vault-reconciler.json`
...
- for user-agent runs on a shared host, do not inspect central deployment secrets such as `arclink.env` or source `bin/common.sh`
...
- do not patch Hermes code for this unless the task is explicitly about adding warm-reload support after verifying the ArcLink rail is already correct
