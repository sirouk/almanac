---
name: arclink-vaults
description: Use to inspect the active ArcLink vault catalog, curate subscribed vs default vaults, inspect the canonical managed-memory payload that feeds plugin context, and trigger subscription refresh without changing qmd retrieval access.
---

# ArcLink Vaults

Use this skill when an enrolled user wants to manage ArcLink vault subscriptions and understand which vaults are represented in the agent's plugin-managed routing context.
...
- For deep content retrieval, use `arclink-qmd-mcp`; vault subscriptions shape
...
- stay within the current user's `HERMES_HOME` plus the shared ArcLink control plane; do not browse other users' home directories or central deployment config files
...
- "refresh my ArcLink vault context"
