...
DEFAULT_SHARED_REPO_DIR="/home/arclink/arclink"
...
REPO_DIR="${ARCLINK_REPO_DIR:-${ARCLINK_SHARED_REPO_DIR:-}}"
if [[ -z "$REPO_DIR" && -f "$SOURCE_REPO_DIR/python/arclink_rpc_client.py" ]]; then
  REPO_DIR="$SOURCE_REPO_DIR"
fi
if [[ -z "$REPO_DIR" && -f "$DEFAULT_SHARED_REPO_DIR/python/arclink_rpc_client.py" ]]; then
  REPO_DIR="$DEFAULT_SHARED_REPO_DIR"
fi
if [[ -z "$REPO_DIR" || ! -f "$REPO_DIR/python/arclink_rpc_client.py" ]]; then
  echo "curate-vaults: cannot locate ArcLink repo with python/arclink_rpc_client.py; set ARCLINK_REPO_DIR" >&2
  exit 2
fi
TOKEN_FILE="${ARCLINK_BOOTSTRAP_TOKEN_FILE:-${ARCLINK_BOOTSTRAP_TOKEN_PATH:-$HERMES_HOME/secrets/arclink-bootstrap-token}}"
JSON_MODE="0"
...
Environment:
  ARCLINK_MCP_URL            Override control-plane MCP URL.
  ARCLINK_BOOTSTRAP_TOKEN_FILE / ARCLINK_BOOTSTRAP_TOKEN_PATH
...
for key in ("arclink-skill-ref", "vault-ref", "qmd-ref", "vault-topology"):
    print(f"\n[{key}]")
...
payload = {
    ...
    "catalog": vaults,
    "managed_memory_keys": [
        key for key in ("arclink-skill-ref", "vault-ref", "qmd-ref", "vault-topology")
        if managed.get(key)
    ],
    "trigger_contract": {
        ...
        "agent_refresh": "arclink-user-agent-refresh rewrites local plugin-managed context state on timer and activation-trigger path changes",
    },
}
