...
After the script returns, an operator copies the PDFs into the vault under
`Projects/<area>/<topic>/`. The `almanac-pdf-ingest.timer` (every 5 minutes)
then auto-converts each PDF to markdown and qmd's `vault-pdf-ingest`
collection picks them up. From that point any agent can answer questions
about the captured material via `knowledge.search-and-fetch`.
