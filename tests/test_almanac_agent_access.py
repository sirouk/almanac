...
def test_access_state_uses_tailscale_port_urls_when_enabled() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            hermes_home.mkdir(parents=True, exist_ok=True)
            insert_agent(control, conn, agent_id="agent-current", unix_user="current", hermes_home=hermes_home)
            access_mod.detect_tailscale_dns_name = lambda: "almanac.example.test"
            ...
            expect(state["dashboard_url"] == f"https://almanac.example.test:{state['dashboard_proxy_port']}/", state)
            expect(state["code_url"] == f"https://almanac.example.test:{state['code_port']}/", state)
...
def test_publish_tailscale_https_uses_dedicated_ports() -> None:
    ...
    expect(updated["dashboard_url"] == "https://almanac.example.test:30011/", updated)
    expect(updated["code_url"] == "https://almanac.example.test:40011/", updated)
...
def test_docker_port_allocation_skips_host_listeners() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    access_mod = load_module(ACCESS_PY, "almanac_agent_access_docker_port_probe")
    old_env = os.environ.copy()
