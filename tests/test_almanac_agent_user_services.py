...
def test_generated_backup_cron_job_follows_backup_state_file() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        backup_cron_script = hermes_home / "scripts" / "almanac_agent_backup.py"
        ...
        expect(backup_job["script"] == "almanac_agent_backup.py", str(backup_job))
        ...
        expect("--user disable --now almanac-user-agent-backup.timer" in log, log)
        expect("--user enable almanac-user-agent-backup.timer" not in log, log)
        expect("--user restart almanac-user-agent-backup.timer" not in log, log)
        expect(
