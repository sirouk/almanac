...
PROXY_PY = REPO / "python" / "almanac_basic_auth_proxy.py"
...
def test_proxy_allows_hermes_bearer_api_calls_after_basic_login() -> None:
    proxy_mod = load_module(PROXY_PY, "almanac_basic_auth_proxy_test")
    with tempfile.TemporaryDirectory() as tmp:
        access_file = Path(tmp) / "almanac-web-access.json"
        access_file.write_text(
            json.dumps({"username": "sirouk", "password": "test-password"}),
            encoding="utf-8",
        )
        ...
        try:
            ...
            expect(headers.get("WWW-Authenticate") == 'Basic realm="Almanac Hermes"', headers)

            status, headers, _body = request(
                proxy.server_port,
                "/",
                headers={"Authorization": basic_header("sirouk", "test-password")},
            )
