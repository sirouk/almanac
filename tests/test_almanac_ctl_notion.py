...
def test_notion_preflight_root_cli_accepts_token_file() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    ctl = load_module(CTL_PY, "almanac_ctl_notion_preflight_token_file_test")
    original_preflight = ctl.preflight_notion_root_children
    original_argv = sys.argv[:]
    old_env = os.environ.copy()
    buffer = io.StringIO()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            os.environ["ALMANAC_SSOT_NOTION_ROOT_PAGE_ID"] = "aaaaaaaa-aaaa-bbbb-bbbb-bbbbbbbbbbbb"
            os.environ["ALMANAC_SSOT_NOTION_TOKEN"] = "secret_from_env"
            os.environ["ALMANAC_SSOT_NOTION_API_VERSION"] = "2026-03-11"
...
def main() -> int:
    ...
    print("PASS all 13 almanac ctl notion tests")
    return 0
