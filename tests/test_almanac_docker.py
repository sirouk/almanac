...
def test_docker_operator_commands_are_present() -> None:
    ...
    expect("docker_reconcile()" in body and "./bin/almanac-ctl org-profile apply --yes" in body, body)
    expect("wait_for_docker_agent_reconcile()" in body and "almanac-vault-reconciler.json" in body, body)
...
def test_docker_agent_supervisor_replaces_user_systemd_units() -> None:
    ...
    expect("run_headless_identity_setup" in supervisor and "almanac_headless_hermes_setup.py" in supervisor, supervisor)
...
def test_docker_health_script_checks_container_runtime() -> None:
    ...
    expect("almanac-managed-context" in body and "SOUL.md" in body, body)
    expect("almanac-vault-reconciler.json" in body, body)
