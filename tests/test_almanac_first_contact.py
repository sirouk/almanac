...
SOURCE_SCRIPT = REPO / "skills" / "almanac-first-contact" / "scripts" / "run-first-contact.sh"
...
def write_fake_almanac_control(path: Path) -> None:
    path.write_text(
        "from __future__ import annotations\n"
        "import json\n"
        "from pathlib import Path\n"
...
def test_first_contact_runs_real_qmd_probe() -> None:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            script_path = repo_dir / "skills" / "almanac-first-contact" / "scripts" / "run-first-contact.sh"
            ...
            write_fake_rpc(bin_dir / "almanac-rpc")
            write_fake_almanac_control(python_dir / "almanac_control.py")
            ...
            result = subprocess.run(
                ...
                env={
                    **os.environ,
                    ...
                    "ALMANAC_BOOTSTRAP_TOKEN_FILE": str(token_file),
                    "ALMANAC_SHARED_REPO_DIR": str(repo_dir),
                    "ALMANAC_AGENTS_STATE_DIR": str(root / "agents-state"),
                    "ALMANAC_NOTION_INDEX_ROOTS": "https://www.notion.so/Root-aaaaaaaaaaaabbbbbbbbbbbbbbbb",
                },
                text=True,
                capture_output=True,
                check=False,
