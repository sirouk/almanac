...
def test_almanac_mcp_tools_advertise_actionable_input_schemas() -> None:
    ...
    expect("configured Almanac root page" in ssot_write["properties"]["target_id"]["description"], str(ssot_write))
...
def test_notion_index_fallback_returns_qmd_markdown_body() -> None:
    mod = load_module(MCP_SERVER, "almanac_mcp_server_notion_qmd_fallback_test")
            ...
            "content": [
                {
                    ...
                    "resource": {
                        ...
                        "mimeType": "text/markdown",
                        "text": "# Replay Page\n\nALMANAC_REPLAY_NOTION_DRI stays retrievable from the index.",
                    },
                }
            ]
        }
    ...
    expect("ALMANAC_REPLAY_NOTION_DRI" in fetched["markdown"], str(fetched))
...
def test_vault_source_metadata_adapts_to_vault_roots_repos_and_pdf_sidecars() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        expect(lowercased_qmd_path["source_host_path"] == "/home/alice/Almanac/TeamDocs/README.md", str(lowercased_qmd_path))
...
def main() -> int:
    ...
    print("PASS all 10 Almanac MCP schema tests")
    return 0
