    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            (hermes_home / "state" / "almanac-web-access.json").write_text(
                json.dumps(
                    {
                        "dashboard_url": "https://almanac.example.test:30042/",
                        "code_url": "https://almanac.example.test:40042/",
                    }
                )
                + "\n",
                encoding="utf-8",
            ...
            expect("Ask vault.search-and-fetch for depth; current lane root is ~/Almanac/Projects" in managed_payload["recall-stubs"], managed_payload["recall-stubs"])
            ...
            expect(f"Vault root: {root / 'home-test' / 'Almanac'}" in managed_payload["vault-ref"], managed_payload["vault-ref"])
            ...
            expect("Hermes dashboard: https://almanac.example.test:30042/" in managed_payload["resource-ref"], managed_payload["resource-ref"])
            ...
            expect("does not require a fixed Projects/Repos taxonomy" in managed_payload["almanac-skill-ref"], managed_payload["almanac-skill-ref"])
            ...
            expect("Agents_Skills/*/skills" in managed_payload["almanac-skill-ref"], managed_payload["almanac-skill-ref"])
            ...
            expect("Raw qmd MCP initialize/session-id details are for explicit Almanac" in managed_payload["qmd-ref"], managed_payload["qmd-ref"])
            expect("retry the\nsame brokered Almanac MCP tool once" in managed_payload["qmd-ref"], managed_payload["qmd-ref"])
