#!/usr/bin/env python3
"""Regression tests for almanac_pin_upgrade_check (the detector + throttle).
...
PIN_DETECTOR = REPO / "python" / "almanac_pin_upgrade_check.py"
...
def _setup_env(scratch_root: Path):
    config_path = scratch_root / "config" / "almanac.env"
...
def test_detector_first_run_inserts_state_and_queues_one_digest_notification() -> None:
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_first_run")
    control = load_module(CONTROL_PY, "almanac_control_pin_first_run")
...
def test_detector_adds_operator_buttons_for_pinned_component_digest() -> None:
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_buttons")
    control = load_module(CONTROL_PY, "almanac_control_pin_buttons")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        ...
        values["ALMANAC_CURATOR_TELEGRAM_ONBOARDING_ENABLED"] = "1"
...
def test_detector_digest_includes_git_commit_release_labels() -> None:
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_release_labels")
    control = load_module(CONTROL_PY, "almanac_control_pin_release_labels")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        old_env = _setup_env(root)
        try:
...
def test_detector_uses_release_version_target_across_git_commit_churn() -> None:
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_release_throttle")
    control = load_module(CONTROL_PY, "almanac_control_pin_release_throttle")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        old_env = _setup_env(root)
        try:
...
def test_detector_silences_after_configured_strikes_against_same_target() -> None:
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_throttle")
    control = load_module(CONTROL_PY, "almanac_control_pin_throttle")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        old_env = _setup_env(root)
        try:
...
def test_detector_resets_throttle_when_target_advances() -> None:
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_advance")
    control = load_module(CONTROL_PY, "almanac_control_pin_advance")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        old_env = _setup_env(root)
        try:
...
def test_detector_clears_state_when_pin_no_longer_lags_upstream() -> None:
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_cleared")
    control = load_module(CONTROL_PY, "almanac_control_pin_cleared")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        old_env = _setup_env(root)
        try:
...
def test_detector_preserves_state_on_transient_upstream_failure() -> None:
    ...
    """
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_transient")
    control = load_module(CONTROL_PY, "almanac_control_pin_transient")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        old_env = _setup_env(root)
        try:
...
def test_detector_preserves_state_on_check_runner_exception_output() -> None:
    ...
    """
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_runner_exception")
    control = load_module(CONTROL_PY, "almanac_control_pin_runner_exception")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        old_env = _setup_env(root)
        try:
...
def test_run_check_marks_partial_nonzero_output_transient() -> None:
    detector = load_module(PIN_DETECTOR, "almanac_pin_upgrade_check_partial_nonzero")
