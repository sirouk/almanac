...
Locks in:
  ...
  - bin/pins.sh round-trips a value (get → set → get) without corrupting
    sibling components.
  - bin/common.sh resolves ALMANAC_HERMES_AGENT_REF and
    ALMANAC_AGENT_CODE_SERVER_IMAGE through pins.json (not the literal
    fallback constants).
"""
...
def test_pins_sh_round_trip_does_not_corrupt_other_components() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        env = {**os.environ, "ALMANAC_PINS_FILE": str(scratch)}
...
def test_common_sh_reads_hermes_pin_from_pins_json() -> None:
    ...
    out = subprocess.check_output([
        "bash", "-c",
        f"set -e; cd {REPO}; "
        "ALMANAC_HERMES_AGENT_REF=0000000000000000000000000000000000000000; "
        "ALMANAC_HERMES_DOCS_REF=1111111111111111111111111111111111111111; "
        "ALMANAC_AGENT_CODE_SERVER_IMAGE=docker.io/example/stale:0.0.0; "
        f"source {COMMON_SH} 2>/dev/null || true; "
        "echo $ALMANAC_HERMES_AGENT_REF; echo $ALMANAC_HERMES_DOCS_REF; echo $ALMANAC_AGENT_CODE_SERVER_IMAGE"
...
def test_component_upgrade_commits_pending_pin_bump_without_git_identity() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        expect(author == "Almanac Upgrade Bot <almanac-upgrade@localhost>", author)
