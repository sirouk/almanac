...
def test_sync_vault_repo_mirrors_marks_shared_vault_checkouts_safe_for_git() -> None:
    ...
    sync runs as the Almanac service user. Git's dubious-ownership protection
    must not prevent the explicit vault repo-sync rail from tending that
    checkout."""
    mod = load_module(CONTROL_PY, "almanac_control_shared_safe_dir_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        write_repos_vault(root)
...
def test_repo_sync_git_disables_interactive_prompts_and_scopes_safe_directory() -> None:
    ...
    mod = load_module(CONTROL_PY, "almanac_control_git_env_test")
    with tempfile.TemporaryDirectory() as tmp:
...
def test_sync_vault_repo_mirrors_records_auth_failure_and_keeps_going() -> None:
    ...
    mod = load_module(CONTROL_PY, "almanac_control_auth_failure_continue_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        old_env = os.environ.copy()
                ...
                repo_sources=[
                    {
                        ...
                        "source_paths": [],
                        "local_repo_paths": [str(root / "vault" / "Repos" / "private")],
                    },
                    {
                        "slug": "public",
                        ...
                        "source_paths": [],
                        "local_repo_paths": [str(root / "vault" / "Repos" / "public")],
                    },
                ],
            )
        finally:
