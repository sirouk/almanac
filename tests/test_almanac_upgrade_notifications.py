...
def test_upgrade_check_uses_configured_upstream_deploy_key_for_ssh_remotes() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            cfg = control.Config.from_env()
            git_env = ctl._upstream_git_env(cfg, "git@github.com:example/almanac.git")
            ...
            expect(
                ctl._upstream_git_env(cfg, "https://github.com/example/almanac.git") is None,
...
def test_upgrade_check_falls_back_to_https_when_operator_deploy_key_is_not_service_readable() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
            ...
            json.dumps(
                {
                    ...
                    "tracked_upstream_repo_url": "git@github.com:example/almanac.git",
                    "tracked_upstream_branch": "main",
                }
            )
            + "\n",
        ...
        def fake_query(repo_url: str, branch: str, env=None):
            ...
            if repo_url == "git@github.com:example/almanac.git":
                raise RuntimeError("operator deploy key is not readable by service user")
            if repo_url == "https://github.com/example/almanac.git":
                return deployed_sha
        ...
        def fake_classify(repo_dir: Path, repo_url: str, branch: str, deployed_commit: str, upstream_commit: str) -> str:
            expect(repo_url == "https://github.com/example/almanac.git", repo_url)
        ...
        try:
            ...
            expect(result["upstream_query_url"] == "https://github.com/example/almanac.git", result)
            expect(result["upstream_transport_fallback"] == "https", result)
            expect(calls == [
                ("git@github.com:example/almanac.git", "main", True),
                ("https://github.com/example/almanac.git", "main", False),
            ], calls)
            job = conn.execute(
...
def test_upgrade_check_git_run_disables_interactive_auth_prompts() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    ctl = load_module(CTL_PY, "almanac_ctl_upgrade_git_run_prompt_env_test")
    seen: dict[str, object] = {}
