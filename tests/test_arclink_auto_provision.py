...
def test_mark_auto_provision_started_claims_only_once_until_finished() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_auto_provision_claim_test")
...
def test_stale_auto_provision_claims_are_reclaimable() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_auto_provision_stale_claim_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_auto_provision_bootstrap_rejects_reserved_unix_user() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_auto_provision_reserved_unix_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_run_one_uses_devnull_stdin_for_headless_init() -> None:
    ...
    control = load_module(CONTROL_PY, "arclink_control_auto_provision_run_one_control")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_stdin_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
        ...
        try:
            ...
            provisioner.get_agent = lambda conn, agent_id: {"hermes_home": str(root / "home" / "user" / ".local" / "share" / "arclink-agent" / "hermes-home")}
...
def test_run_as_user_scrubs_ambient_env() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_env_scrub_test")
    calls: list[dict[str, object]] = []
    ...
    try:
        provisioner._run_as_user(
            ...
            uid=1234,
            hermes_home=Path("/home/scrubtest/.local/share/arclink-agent/hermes-home"),
            cmd=["systemctl", "--user", "is-active", "dummy.service"],
            extra={"ARCLINK_AGENT_VAULT_DIR": "/srv/arclink/vault"},
        )
        ...
        expect(env.get("HERMES_HOME") == "/home/scrubtest/.local/share/arclink-agent/hermes-home", env)
        expect(env.get("ARCLINK_AGENT_VAULT_DIR") == "/srv/arclink/vault", env)
