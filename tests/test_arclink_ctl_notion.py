...
def test_redact_identity_rows_masks_emails_by_default() -> None:
    ...
    ctl = load_module(CTL_PY, "arclink_ctl_notion_redact_test")
    rows = [
        {
            "unix_user": "alex",
    ...
    expect(scrubbed[0]["claimed_notion_email"] == "a***@example.com", str(scrubbed))
    ...
    expect(shown[0]["claimed_notion_email"] == "alex@example.com", str(shown))
...
def test_manual_verify_notion_user_fetches_and_validates_email() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    ctl = load_module(CTL_PY, "arclink_ctl_notion_verify_test")
    ...
    try:
        ...
        expect(notion_user_email == "alex@example.com", notion_user_email)
        try:
...
def test_suspended_identity_cannot_be_reverified_without_unsuspend() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_identity_transition_test")
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            control.suspend_agent_identity(conn, unix_user="alex")
            try:
            ...
            expect(refreshed["verification_source"] == "notion-live-check:alex@example.com", str(refreshed))
...
def test_notion_suspend_and_unsuspend_cli_audit() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
        control = load_module(CONTROL_PY, "arclink_control_ctl_suspend_test")
        old_env = os.environ.copy()
        os.environ["ARCLINK_CONFIG_FILE"] = str(config_path)
        try:
            cfg = control.Config.from_env()
        ...
        suspended = subprocess.run(
            [sys.executable, str(CTL_PY), "--json", "notion", "suspend", "alex", "--actor", "operator", "--reason", "test suspend"],
            cwd=str(REPO),
            env=env,
            text=True,
            capture_output=True,
        ...
        unsuspended = subprocess.run(
            [sys.executable, str(CTL_PY), "--json", "notion", "unsuspend", "alex", "--actor", "operator", "--reason", "test unsuspend"],
            cwd=str(REPO),
            env=env,
            text=True,
            capture_output=True,
        ...
        try:
            cfg = control.Config.from_env()
            conn = control.connect_db(cfg)
            identity = control.get_agent_identity(conn, unix_user="alex")
            ...
            audit_rows = control.list_ssot_access_audit(conn, unix_user="alex", limit=5)
...
def test_connect_db_migrates_legacy_notion_claim_nonce_column() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_notion_claim_migration_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
        db_path = root / "state" / "arclink-control.sqlite3"
        ...
        try:
            ...
            raw.executescript(
                ...
                INSERT INTO notion_identity_claims (
                  ...
                  'nclaim_old', 'session-1', 'agent-alex', 'alex', 'alex@example.com',
            ...
            expect(row is not None and row["claimed_notion_email"] == "alex@example.com", str(dict(row) if row else {}))
...
def test_expire_stale_notion_identity_claims_marks_pending_claims_expired() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_expire_claims_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_notion_override_cli_round_trip() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
        control = load_module(CONTROL_PY, "arclink_control_ctl_override_test")
        old_env = os.environ.copy()
        os.environ["ARCLINK_CONFIG_FILE"] = str(config_path)
        try:
            cfg = control.Config.from_env()
        ...
        expect(set_payload["notion_user_email"] == "a***@example.com", str(set_payload))
        ...
        expect(listed["overrides"][0]["notion_user_email"] == "a***@example.com", str(listed))
        ...
        expect(sensitive["overrides"][0]["notion_user_email"] == "alex@example.com", str(sensitive))
        expect(
        ...
        clear_result = subprocess.run(
            [sys.executable, str(CTL_PY), "--json", "notion", "override-clear", "alex"],
            cwd=str(REPO),
            env=env,
            text=True,
            capture_output=True,
...
def test_notion_preflight_root_cli_uses_root_page_id() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    ctl = load_module(CTL_PY, "arclink_ctl_notion_preflight_test")
    ...
    try:
        os.environ["ARCLINK_SSOT_NOTION_ROOT_PAGE_ID"] = "aaaaaaaa-aaaa-bbbb-bbbb-bbbbbbbbbbbb"
        os.environ["ARCLINK_SSOT_NOTION_TOKEN"] = "secret_test"
        os.environ["ARCLINK_SSOT_NOTION_API_VERSION"] = "2026-03-11"
...
def test_notion_preflight_root_cli_accepts_token_file() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    ctl = load_module(CTL_PY, "arclink_ctl_notion_preflight_token_file_test")
    original_preflight = ctl.preflight_notion_root_children
    original_argv = sys.argv[:]
    old_env = os.environ.copy()
    buffer = io.StringIO()
    calls: list[dict[str, str]] = []
    with tempfile.TemporaryDirectory() as tmp:
        token_path = Path(tmp) / "notion-token"
        token_path.write_text("secret_from_file\n", encoding="utf-8")
        try:
            os.environ["ARCLINK_SSOT_NOTION_ROOT_PAGE_ID"] = "aaaaaaaa-aaaa-bbbb-bbbb-bbbbbbbbbbbb"
            os.environ["ARCLINK_SSOT_NOTION_TOKEN"] = "secret_from_env"
            os.environ["ARCLINK_SSOT_NOTION_API_VERSION"] = "2026-03-11"
            ...
            ctl.preflight_notion_root_children = fake_preflight
            sys.argv = [str(CTL_PY), "--json", "notion", "preflight-root", "--token-file", str(token_path)]
            with contextlib.redirect_stdout(buffer):
                ctl.main()
        finally:
            ctl.preflight_notion_root_children = original_preflight
            ...
            os.environ.clear()
            os.environ.update(old_env)
    expect(calls and calls[0]["token"] == "secret_from_file", str(calls))
    print("PASS test_notion_preflight_root_cli_accepts_token_file")
...
def test_notion_webhook_cli_requires_force_and_tracks_arm_window() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        values["ARCLINK_NOTION_WEBHOOK_PUBLIC_URL"] = "https://hooks.example.com/notion/webhook"
...
def test_notion_webhook_cli_refuses_non_owner_control_caller() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_notion_webhook_cli_owner_gate_test")
    ctl = load_module(CTL_PY, "arclink_ctl_notion_webhook_cli_owner_gate_test")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            try:
                ...
                expect("owner of the ArcLink control files" in str(exc), str(exc))
            else:
...
def test_notion_webhook_status_show_secret_requires_owner_and_returns_token() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_notion_webhook_show_secret_test")
    ctl = load_module(CTL_PY, "arclink_ctl_notion_webhook_show_secret_test")
...
def test_notion_webhook_confirm_verified_records_operator_confirmation() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_notion_webhook_confirm_verified_test")
    ctl = load_module(CTL_PY, "arclink_ctl_notion_webhook_confirm_verified_test")
...
def main() -> int:
    ...
    test_notion_preflight_root_cli_accepts_token_file()
    ...
    print("PASS all 13 arclink ctl notion tests")
    return 0
