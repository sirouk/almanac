...
PROVISIONER_PY = REPO / "python" / "arclink_enrollment_provisioner.py"
...
def insert_agent(mod, conn, *, agent_id: str, unix_user: str) -> None:
    now = mod.utc_now_iso()
    hermes_home = Path("/home") / unix_user / ".local" / "share" / "arclink-agent" / "hermes-home"
    conn.execute(
        """
        INSERT INTO agents (
...
def test_discord_completion_handoff_queues_root_dm_action() -> None:
    ...
    curator_text = (PYTHON_DIR / "arclink_curator_discord_onboarding.py").read_text(encoding="utf-8")
    ...
    expect('if command == "/upgrade":' in curator_text, "Curator Discord should accept typed /upgrade in the operator channel")
...
def test_retry_contact_force_resends_discord_handoff_after_sent_marker() -> None:
    ...
    control = load_module(CONTROL_PY, "arclink_control_retry_contact_force_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_retry_contact_force_test")
...
def test_completion_bundle_send_is_idempotent() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_completion_idempotent_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_completion_idempotent_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_completed_backup_setup_prompt_backfill_is_idempotent_for_chat_onboarding() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_backup_prompt_backfill_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_backup_prompt_backfill_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_webhook_verified_claim_finishes_onboarding_and_sends_completion_bundle() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_provisioner_webhook_complete_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_webhook_complete_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_gateway_failures_notify_user_with_provision_error_status() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_gateway_failure_notify_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_gateway_failure_notify_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_operator_upgrade_actions_run_root_upgrade_and_notify_operator() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_operator_upgrade_queue_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_operator_upgrade_queue_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        ...
        try:
            ...
            def fake_run_host_upgrade(cfg, *, log_path):
                log_path.write_text("Fetching ArcLink upstream...\nArcLink upgrade complete.\n", encoding="utf-8")
...
def test_operator_pin_upgrade_actions_run_component_upgrade_and_notify_operator() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_operator_pin_upgrade_queue_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_operator_pin_upgrade_queue_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        values = config_values(root)
...
def test_run_pin_upgrade_action_pins_targets_then_runs_deploy_upgrade() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_pin_upgrade_command_sequence_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_pin_upgrade_command_sequence_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        repo_dir = root / "repo"
...
def test_operator_upgrade_stale_running_action_fails_closed() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_operator_upgrade_stale_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_operator_upgrade_stale_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        values = config_values(root)
...
def test_operator_action_finish_retries_transient_sqlite_lock() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_operator_finish_retry_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_operator_finish_retry_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
        old_env = os.environ.copy()
        os.environ["ARCLINK_CONFIG_FILE"] = str(config_path)
        os.environ["ARCLINK_OPERATOR_SQLITE_RETRY_SECONDS"] = "5"
        try:
            cfg = control.Config.from_env()
            conn = control.connect_db(cfg)
            action_row, created = control.request_operator_action(
...
def test_run_host_upgrade_seeds_home_when_missing() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_run_host_upgrade_home_test")
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_run_host_upgrade_home_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_run_host_upgrade_routes_to_docker_upgrade_in_docker_mode() -> None:
    ...
    provisioner = load_module(PROVISIONER_PY, "arclink_enrollment_provisioner_run_host_upgrade_docker_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
...
def test_install_system_services_does_not_self_deadlock_on_active_oneshots() -> None:
    ...
    expect(
        "start_system_service_if_idle arclink-notion-claim-poll.service" in text,
        text,
    )
    expect(
