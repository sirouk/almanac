...
SCRIPT = REPO / "python" / "arclink_headless_hermes_setup.py"
...
def write_arclink_config(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "\n".join(
            [
                "ARCLINK_ORG_NAME='Acme Labs'",
                "ARCLINK_ORG_MISSION='Make serious research more legible and actionable.'",
                "ARCLINK_ORG_PRIMARY_PROJECT='Hermes deployment lane'",
                "ARCLINK_ORG_TIMEZONE='America/New_York'",
                "ARCLINK_ORG_QUIET_HOURS='22:00-08:00 weekdays'",
                "",
            ]
        ),
        encoding="utf-8",
...
def test_identity_only_writes_soul_and_dual_surface_prefill_config() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        home = root / "home"
        org_skill_root = home / "ArcLink" / "Agents_Skills" / "ralphie" / "skills"
        ...
        arclink_config = root / "arclink-priv" / "config" / "arclink.env"
        hermes_config = root / "fake-hermes-config.json"
        hermes_config.write_text(
            json.dumps(
                {
                    "plugins": {"disabled": ["arclink-managed-context", "other-plugin"]}
                },
                indent=2,
                sort_keys=True,
            )
        ...
        expect(prefill_path == hermes_home / "state" / "arclink-prefill-messages.json", payload)
        expect(identity_state_path == hermes_home / "state" / "arclink-identity-context.json", payload)
        ...
        expect("Hermes already injects your durable identity from HERMES_HOME/SOUL.md" in prefill_messages[0]["content"], prefill_messages)
        expect("read it first" not in prefill_messages[0]["content"], prefill_messages)
        ...
        expect("arclink-managed-context plugin can inject refreshed local ArcLink context into future turns" in prefill_messages[0]["content"], prefill_messages)
        expect("the next session, /reset, or a gateway restart" not in prefill_messages[0]["content"], prefill_messages)
        expect("arclink-qmd-mcp for vault retrieval" not in prefill_messages[0]["content"], prefill_messages)
        ...
        expect("arclink-managed-context" not in hermes_cfg["plugins"]["disabled"], hermes_cfg)
...
def test_render_soul_fails_loudly_on_unknown_placeholder() -> None:
    module = load_module(SCRIPT, "arclink_headless_setup_render_guard_test")
...
def test_reasoning_effort_is_written_to_hermes_agent_config() -> None:
    module = load_module(SCRIPT, "arclink_headless_setup_reasoning_effort_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        fake_pkg_root = write_fake_hermes_cli(root)
        hermes_config = root / "fake-hermes-config.json"
...
def test_anthropic_oauth_seed_writes_claude_code_credentials_and_clears_env_tokens() -> None:
    module = load_module(SCRIPT, "arclink_headless_setup_anthropic_claude_code_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        fake_pkg_root = write_fake_hermes_cli(root)
        hermes_config = root / "fake-hermes-config.json"
