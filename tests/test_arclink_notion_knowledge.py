...
def config_values(root: Path) -> dict[str, str]:
    return {
        ...
        "ARCLINK_SSOT_NOTION_ROOT_PAGE_URL": "https://www.notion.so/workspace-root-aaaaaaaaaaaabbbbbbbbbbbbbbbb",
        "ARCLINK_SSOT_NOTION_ROOT_PAGE_ID": "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa",
        "ARCLINK_SSOT_NOTION_SPACE_URL": "https://www.notion.so/workspace-root-aaaaaaaaaaaabbbbbbbbbbbbbbbb",
        "ARCLINK_SSOT_NOTION_SPACE_ID": "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa",
        "ARCLINK_SSOT_NOTION_SPACE_KIND": "page",
        "ARCLINK_SSOT_NOTION_TOKEN": "secret_test",
        "ARCLINK_SSOT_NOTION_API_VERSION": "2026-03-11",
        "ARCLINK_NOTION_INDEX_ROOTS": "https://www.notion.so/workspace-root-aaaaaaaaaaaabbbbbbbbbbbbbbbb",
        "ARCLINK_NOTION_INDEX_COLLECTION_NAME": "notion-shared",
...
def test_sync_shared_notion_index_indexes_page_tree_into_qmd_markdown_docs() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_notion_index_sync_test")
...
def test_notion_search_fetch_and_query_use_shared_index_and_live_reads() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_notion_knowledge_tools_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_process_pending_notion_events_queues_full_reindex_for_data_source_and_file_upload() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_notion_event_pipeline_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_incremental_reindex_parent_walks_brand_new_page_under_known_root() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_notion_parent_walk_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_incremental_reindex_removes_trashed_page_docs() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_notion_trashed_page_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_today_plate_discovers_child_task_databases_and_filters_owner_items() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_today_plate_discovery_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_today_plate_page_scoped_without_task_db_falls_back_to_qmd_message() -> None:
    mod = load_module(CONTROL_PY, "arclink_control_today_plate_no_task_db_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
