...
def test_verify_notion_command_reopens_latest_completed_session() -> None:
    ...
    control = load_module(CONTROL_PY, "arclink_control_onboarding_notion_resume_test")
    onboarding = load_module(ONBOARDING_PY, "arclink_onboarding_notion_resume_test")
...
def test_awaiting_notion_access_skip_returns_completion_bundle() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_onboarding_notion_skip_test")
    onboarding = load_module(ONBOARDING_PY, "arclink_onboarding_notion_skip_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_awaiting_notion_access_ready_moves_to_email_step() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_onboarding_notion_access_ready_test")
    onboarding = load_module(ONBOARDING_PY, "arclink_onboarding_notion_access_ready_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_awaiting_notion_email_starts_claim_and_moves_to_verification() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_onboarding_notion_claim_test")
    onboarding = load_module(ONBOARDING_PY, "arclink_onboarding_notion_claim_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
        ...
        try:
            ...
            def fake_start_claim(conn_arg, *, session_id: str, agent_id: str, unix_user: str, claimed_notion_email: str, urlopen_fn=None):
                ...
                expect(agent_id == "agent-alex", agent_id)
                expect(unix_user == pwd.getpwuid(os.getuid()).pw_name, unix_user)
                expect(claimed_notion_email == "alex@example.com", claimed_notion_email)
                return {
            ...
            expect(str(answers.get("notion_claim_email") or "") == "alex@example.com", str(answers))
...
def test_verify_notion_command_reissues_pending_claim_with_same_email() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_onboarding_notion_reissue_claim_test")
    onboarding = load_module(ONBOARDING_PY, "arclink_onboarding_notion_reissue_claim_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_verify_notion_command_reissues_expired_claim_with_same_email() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_onboarding_notion_expired_claim_test")
    onboarding = load_module(ONBOARDING_PY, "arclink_onboarding_notion_expired_claim_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
...
def test_awaiting_notion_verification_skip_marks_claim_and_returns_completion_bundle() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_onboarding_notion_skip_claim_test")
    onboarding = load_module(ONBOARDING_PY, "arclink_onboarding_notion_skip_claim_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(config_path, config_values(root))
