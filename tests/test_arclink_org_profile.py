...
def test_ultimate_example_profile_applies_to_state_vault_and_agent_memory() -> None:
    control = load_module(REPO / "python" / "arclink_control.py", "arclink_control_org_profile_test")
    org_profile = load_module(REPO / "python" / "arclink_org_profile.py", "arclink_org_profile_test")
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            alex_home = root / "home-alex" / ".local" / "share" / "arclink-agent" / "hermes-home"
            ren_home = root / "home-ren" / ".local" / "share" / "arclink-agent" / "hermes-home"
            ...
            identity = json.loads((alex_home / "state" / "arclink-identity-context.json").read_text(encoding="utf-8"))
            ...
            state_payload = json.loads((alex_home / "state" / "arclink-vault-reconciler.json").read_text(encoding="utf-8"))
            ...
            expect("Repo: northstar-demo/arclink-demo" in state_payload["user-responsibilities"], state_payload)
...
def test_human_owner_modules_and_seed_checksums_are_distributed() -> None:
    org_profile = load_module(REPO / "python" / "arclink_org_profile.py", "arclink_org_profile_modules_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
...
def test_explicit_identity_profile_link_orients_arbitrary_agent_names() -> None:
    control = load_module(REPO / "python" / "arclink_control.py", "arclink_control_org_profile_identity_test")
    org_profile = load_module(REPO / "python" / "arclink_org_profile.py", "arclink_org_profile_identity_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        values = {
        ...
        try:
            ...
            conn = control.connect_db(cfg)
            now = control.utc_now_iso()
            hermes_home = root / "home-random" / ".local" / "share" / "arclink-agent" / "hermes-home"
...
def test_unmatched_agent_receives_org_baseline_without_personalization() -> None:
    control = load_module(REPO / "python" / "arclink_control.py", "arclink_control_org_profile_unmatched_baseline_test")
    org_profile = load_module(REPO / "python" / "arclink_org_profile.py", "arclink_org_profile_unmatched_baseline_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        values = {
        ...
        try:
            ...
            conn = control.connect_db(cfg)
            now = control.utc_now_iso()
            hermes_home = root / "home-new" / ".local" / "share" / "arclink-agent" / "hermes-home"
            hermes_home.mkdir(parents=True)
            insert_agent(
                conn,
                agent_id="agent-new",
...
def test_agent_identity_rejects_duplicate_org_profile_person_links() -> None:
    control = load_module(REPO / "python" / "arclink_control.py", "arclink_control_duplicate_profile_link_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = root / "config" / "arclink.env"
        write_config(
...
def test_builder_starter_profile_covers_operational_rails() -> None:
    org_profile = load_module(REPO / "python" / "arclink_org_profile.py", "arclink_org_profile_builder_profile_test")
    builder = load_module(REPO / "python" / "arclink_org_profile_builder.py", "arclink_org_profile_builder_test")
    with tempfile.TemporaryDirectory() as tmp:
...
def test_generated_vault_render_path_is_vault_relative_and_source_display_sanitized() -> None:
    org_profile = load_module(REPO / "python" / "arclink_org_profile.py", "arclink_org_profile_path_safety_test")
    builder = load_module(REPO / "python" / "arclink_org_profile_builder.py", "arclink_org_profile_path_builder_test")
    profile = builder.profile_starter()
    validation = org_profile.validate_profile(profile)
    expect(validation["valid"], validation)
    ...
    rendered = org_profile.render_vault_profile(
        profile,
        source_path=Path("/home/example/arclink/arclink-priv/config/org-profile.yaml"),
    )
...
def test_org_profile_shared_vault_render_omits_people_when_policy_is_group_visible() -> None:
    org_profile = load_module(REPO / "python" / "arclink_org_profile.py", "arclink_org_profile_render_privacy_test")
