...
def test_arclink_dashboard_plugins_expose_sanitized_access_state() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (workspace_home / "Vault" / "projects").mkdir(parents=True, exist_ok=True)
        (workspace_home / "Vault" / "projects" / "alpha.md").write_text("# Alpha\n", encoding="utf-8")
        (workspace_home / "Vault" / "projects" / "beta.txt").write_text("Beta\n", encoding="utf-8")
        (workspace_home / "Vault" / "projects" / ".git").mkdir(parents=True, exist_ok=True)
        (workspace_home / "Vault" / "projects" / ".git" / "config").write_text("[core]\n", encoding="utf-8")
        (workspace_home / "Vault" / ".ssh").mkdir(parents=True, exist_ok=True)
        (workspace_home / "Vault" / ".ssh" / "id_rsa").write_text("PRIVATE\n", encoding="utf-8")
        ...
        try:
            ...
            expect("synced across ArcPods and machines" in root_map["fleet"]["tooltip"], str(root_map["fleet"]))
            ...
            projects_item = next((item for item in drive_items["items"] if item["name"] == "projects"), None)
            expect(projects_item is not None, str(drive_items))
...
def test_arclink_drive_code_share_request_broker_contract() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            expect("hermes-drive-tree-count" in drive_body, "Drive tree should expose the same folder count slot as Code navigation")
            expect("child_count" in drive_body, "Drive tree should render folder counts supplied by the dashboard API")
...
def test_arclink_drive_browser_exposes_roots_breadcrumbs_and_trash_restore() -> None:
    ...
    expect("setPreviewFullscreenChrome(state.previewFullscreen)" in body, "Drive fullscreen preview should suppress host Hermes chrome")
    expect("#app-sidebar" in body and "[role='navigation']" in body and " nav," in body, "Drive fullscreen preview should hide host navigation chrome")
    expect("window.top" in body and "addDoc(window.top.document)" in body, "Drive fullscreen preview should reach the top Hermes shell when nested")
    expect('doc.addEventListener("keydown", closeFullscreen, true)' in body and 'event.key !== "Escape"' in body, "Drive fullscreen preview should close on Escape")
    expect("tabIndex: -1" in body and "onKeyDown: function (event)" in body, "Drive fullscreen preview should be focusable for Escape handling")
...
def test_arclink_code_browser_opens_source_control_changes_as_diffs() -> None:
    ...
    expect("#app-sidebar" in body and "[role='navigation']" in body and " nav," in body, "Code fullscreen preview should hide host navigation chrome")
    expect("window.top" in body and "addDoc(window.top.document)" in body, "Code fullscreen preview should reach the top Hermes shell when nested")
    expect('doc.addEventListener("keydown", closeFullscreen, true)' in body and 'event.key !== "Escape"' in body, "Code fullscreen preview should close on Escape")
    expect("tabIndex: -1" in body and "onKeyDown: function (event)" in body, "Code fullscreen preview should be focusable for Escape handling")
    ...
    expect("z-index: 2147483000" in style, "Code fullscreen preview should sit above the Hermes nav")
...
def test_arclink_terminal_browser_exposes_persistent_session_controls() -> None:
    ...
    expect("+Machine" not in body and '"New machine terminal"' not in body, "Terminal UI must not duplicate the workspace shell with a redundant +Machine button")
    expect("+SSH" not in body, "Terminal UI must not imply remote SSH for the local machine shell")
    expect('"New workspace shell"' in body and "+Shell" in body, "Terminal UI should expose the single workspace shell button")
