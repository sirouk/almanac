...
def test_public_bot_action_catalog_has_real_platform_commands() -> None:
    ...
    expect("add_agent" in telegram_names, str(telegram))
    expect("retire_agent" in telegram_names, str(telegram))
    expect("share_create" in telegram_names, str(telegram))
    expect("share_approve" in telegram_names, str(telegram))
    expect("share_deny" in telegram_names, str(telegram))
    expect("share_accept" in telegram_names, str(telegram))
    ...
    expect("raven_add_agent" in captain_names and "raven_retire_agent" in captain_names, str(captain_names))
    expect("raven_share_create" in captain_names and "raven_share_approve" in captain_names, str(captain_names))
    ...
    expect(bots.ARCLINK_PUBLIC_BOT_SHARE_ACTION_RE.match("/share_approve share_" + "a" * 32), "share_approve alias should route")

    discord = bots.arclink_public_bot_discord_application_commands()
    discord_names = {item["name"] for item in discord}
    expect({"arclink", "raven", "agent", "connect-notion", "config-backup", "pair-channel", "link-channel", "raven-name", "wrapped-frequency", "upgrade-hermes", "agents", "name", "plan", "add-agent", "retire-agent", "share-create", "share-approve", "share-deny", "share-accept"} <= discord_names, str(discord_names))
...
def test_public_bot_connect_notion_waits_for_credential_acknowledgement() -> None:
    ...
    expect("/raven credentials" in blocked.reply and "confirm storage" in blocked.reply, blocked.reply)
...
def test_public_bot_credentials_reveal_and_ack_dashboard_password() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            expect("Username: arc-credpod@example.test" in revealed.telegram_reply, revealed.telegram_reply)
