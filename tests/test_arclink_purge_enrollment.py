...
def test_user_purge_enrollment_removes_completed_state_and_files() -> None:
    ...
    control = load_module(CONTROL_PY, "arclink_control_purge_enrollment_test")
    ctl = load_module(CTL_PY, "arclink_ctl_purge_enrollment_test")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            hermes_home = root / "home-alex" / ".local" / "share" / "arclink-agent" / "hermes-home"
            ...
            access_state_path.write_text(
                json.dumps(
                    {
                        "agent_id": agent_id,
                        "code_container_name": "arclink-agent-code-agent-alex",
            ...
            repo_checkout = cfg.state_dir / "repo-sync" / "checkouts" / "alex-arclink"
            repo_checkout.mkdir(parents=True, exist_ok=True)
            (repo_checkout / "README.md").write_text("repo\n", encoding="utf-8")
            repo_mirror = cfg.vault_dir / "Repos" / "_mirrors" / "alex-arclink"
...
def test_connect_db_repairs_legacy_notification_outbox_before_creating_retry_index() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_legacy_notification_outbox_repair_test")
...
def test_connect_db_repairs_legacy_ssot_pending_writes_before_creating_expiry_index() -> None:
    if str(PYTHON_DIR) not in sys.path:
        sys.path.insert(0, str(PYTHON_DIR))
    control = load_module(CONTROL_PY, "arclink_control_legacy_ssot_pending_writes_repair_test")
