...
def init_git_repo(path: Path, branch: str = "main") -> None:
    ...
    subprocess.run(["git", "-C", str(path), "config", "user.name", "ArcLink Test"], check=True, capture_output=True, text=True)
    subprocess.run(["git", "-C", str(path), "config", "user.email", "arclink-test@example.com"], check=True, capture_output=True, text=True)
...
def test_discover_vault_repo_sources_ignores_markdown_url_mentions() -> None:
    ...
    mod = load_module(CONTROL_PY, "arclink_control_repo_discovery_no_urlmining")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        repos_dir = write_repos_vault(root)
        human_note = repos_dir / "arclink.md"
        human_note.write_text(
            "# ArcLink\n\nUseful reference: https://github.com/example/arclink\n"
...
def test_discover_vault_repo_sources_finds_local_git_checkouts() -> None:
    ...
    mod = load_module(CONTROL_PY, "arclink_control_repo_local_discovery_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        ...
        try:
            ...
            expect(source["local_repo_paths"] == [str(local_repo.resolve())], str(source))
        finally:
            os.environ.clear()
            os.environ.update(old_env)
...
def test_discover_vault_repo_sources_skips_pinned_sync_trees_and_legacy_mirrors() -> None:
    ...
    by .arclink-source.json) or inside the legacy Repos/_mirrors/ subtree,
    even if they happen to contain a .git/ directory."""
    mod = load_module(CONTROL_PY, "arclink_control_repo_discovery_skips_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        write_repos_vault(root)
        ...
        try:
            ...
            expect(discovered[0]["local_repo_paths"] == [str(legit_repo.resolve())], str(discovered[0]))
        finally:
            os.environ.clear()
            os.environ.update(old_env)
...
def test_sync_vault_repo_mirrors_hard_resets_to_origin_overwriting_local_changes() -> None:
    ...
    """
    mod = load_module(CONTROL_PY, "arclink_control_hard_reset_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        write_repos_vault(root)
                ...
                repo_sources=[
                    {
                        ...
                        "source_paths": [],
                        "local_repo_paths": [str(local_repo.resolve())],
                    }
                ],
            )
...
def test_sync_vault_repo_mirrors_honors_explicit_remote_override() -> None:
    ...
    mod = load_module(CONTROL_PY, "arclink_control_remote_override_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        write_repos_vault(root)
...
def test_sync_vault_repo_mirrors_reports_failure_on_detached_head_and_keeps_going() -> None:
    ...
    mod = load_module(CONTROL_PY, "arclink_control_detached_head_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        write_repos_vault(root)
                ...
                repo_sources=[
                    {
                        ...
                        "source_paths": [],
                        "local_repo_paths": [str(good_repo.resolve())],
                    },
                    {
                        "canonical_url": "https://github.com/example/bad",
                        "remote_url": str(good_bare),
                        "source_paths": [],
                        "local_repo_paths": [str(bad_repo.resolve())],
                    },
                ],
            )
...
def test_sync_vault_repo_mirrors_marks_shared_vault_checkouts_safe_for_git() -> None:
    """Shared vault checkouts may be owned by an enrolled user while the hourly
    sync runs as the ArcLink service user. Git's dubious-ownership protection
    must not prevent the explicit vault repo-sync rail from tending that
    checkout."""
    mod = load_module(CONTROL_PY, "arclink_control_shared_safe_dir_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        write_repos_vault(root)
        local_repo = root / "vault" / "Clients" / "Acme" / "shared-demo"
        bare_remote, seed_repo = make_clone_with_remote(root, clone_path=local_repo)

        (seed_repo / "README.md").write_text("shared vault update\n", encoding="utf-8")
        commit_all(seed_repo, "upstream shared update")
        subprocess.run(["git", "-C", str(seed_repo), "push", "origin", "main"], check=True, capture_output=True, text=True)
        ...
        try:
            cfg = mod.Config.from_env()
            discovered = mod.discover_vault_repo_sources(cfg)
            expect(len(discovered) == 1, f"expected one discovered shared checkout, got {discovered}")
            expect(discovered[0]["remote_url"] == str(bare_remote), str(discovered[0]))
            expect(mod._repo_sync_current_branch(local_repo) == "main", "expected shared checkout branch inspection to work")

            os.environ.pop("GIT_TEST_ASSUME_DIFFERENT_OWNER", None)
            conn = mod.connect_db(cfg)
            mod.reload_vault_definitions(conn, cfg)
            result = mod.sync_vault_repo_mirrors(conn, cfg)
            expect(result["repos_total"] == 1, str(result))
            expect(result["repos_failed"] == [], str(result))
            expect((local_repo / "README.md").read_text(encoding="utf-8") == "shared vault update\n", "expected shared checkout update")
        finally:
            os.environ.clear()
            os.environ.update(old_env)

        print("PASS test_sync_vault_repo_mirrors_marks_shared_vault_checkouts_safe_for_git")


def test_repo_sync_git_disables_interactive_prompts_and_scopes_safe_directory() -> None:
    """Repo sync must fail fast on credentialless remotes and must not require
    operators to write global safe.directory exceptions."""
    mod = load_module(CONTROL_PY, "arclink_control_git_env_test")
    with tempfile.TemporaryDirectory() as tmp:
        repo_dir = Path(tmp) / "vault" / "Repos" / "demo"
        ...
        os.environ["GIT_ASKPASS"] = "/tmp/interactive-askpass"
        os.environ.pop("GIT_SSH_COMMAND", None)
        mod.subprocess.run = fake_run
        try:
            result = mod._repo_sync_git("remote", "get-url", "origin", cwd=repo_dir, timeout=12)
        ...
        print("PASS test_repo_sync_git_disables_interactive_prompts_and_scopes_safe_directory")


def test_sync_vault_repo_mirrors_records_auth_failure_and_keeps_going() -> None:
    """A private repo without usable credentials should be recorded as a repo
    failure while later vault checkouts continue to sync."""
    mod = load_module(CONTROL_PY, "arclink_control_auth_failure_continue_test")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        config_path = make_config(root)
        old_env = os.environ.copy()
        os.environ["ARCLINK_CONFIG_FILE"] = str(config_path)
        original_pull = mod._repo_sync_pull_local_repo

        def fake_pull(repo_dir: Path, remote_url: str = "") -> dict[str, object]:
            if repo_dir.name == "private":
                raise RuntimeError("fatal: could not read Username for 'https://github.com': prompts disabled")
            return {
                "branch": "main",
                "before_commit": "a" * 40,
                "commit": "b" * 40,
                "changed_paths": [str(repo_dir / "README.md")],
                "changed_count": 1,
            }

        mod._repo_sync_pull_local_repo = fake_pull
        try:
            cfg = mod.Config.from_env()
            conn = mod.connect_db(cfg)
            result = mod.sync_vault_repo_mirrors(
                ...
                repo_sources=[
                    {
                        "slug": "private",
                        "canonical_url": "https://github.com/example/private",
                        "remote_url": "https://github.com/example/private.git",
                        "source_paths": [],
                        "local_repo_paths": [str((root / "vault" / "Repos" / "private").resolve())],
                    },
                    {
                        "slug": "public",
                        "canonical_url": "https://github.com/example/public",
                        "remote_url": "https://github.com/example/public.git",
                        "source_paths": [],
                        "local_repo_paths": [str((root / "vault" / "Repos" / "public").resolve())],
                    },
                ],
            )
        finally:
            mod._repo_sync_pull_local_repo = original_pull
            os.environ.clear()
            os.environ.update(old_env)

        expect(result["repos_total"] == 2, str(result))
        expect(result["repos_synced"] == ["public"], str(result))
        expect(len(result["repos_failed"]) == 1, str(result))
        expect("could not read Username" in result["repos_failed"][0], str(result))
        expect(any(path.endswith("Repos/public/README.md") for path in result["changed_paths"]), str(result))
