...
def test_telegram_handle_update_through_bot_contract() -> None:
    ...
    expect("Founders Offer" in result["text"], result["text"])
    expect("3X Scale Plan" in result["text"], result["text"])
...
def test_telegram_fake_transport_polling() -> None:
    ...
    expect("Founders Offer" in transport.sent_messages[0]["text"], transport.sent_messages[0]["text"])
    expect("Captain Test Buyer" in transport.sent_messages[1]["text"], transport.sent_messages[1]["text"])
    expect("Founders Offer" in transport.sent_messages[1]["text"], transport.sent_messages[1]["text"])
    expect("Captain Test Buyer" in transport.sent_messages[2]["text"], transport.sent_messages[2]["text"])
    expect("Founders Offer $149/mo" in str(transport.sent_messages[2].get("reply_markup", {})), str(transport.sent_messages[2]))
    expect("3X Scale Plan $275/mo" in str(transport.sent_messages[2].get("reply_markup", {})), str(transport.sent_messages[2]))
...
def test_telegram_registers_public_bot_actions() -> None:
    ...
    expect("add_agent" in command_sets[0], str(command_sets))
    expect("retire_agent" in command_sets[0], str(command_sets))
    expect("share_create" in command_sets[0], str(command_sets))
    expect("share_approve" in command_sets[0], str(command_sets))
...
def test_telegram_registers_operator_command_scope() -> None:
    ...
    expect("operator_fleet" in command_names, str(command_names))
    ...
    expect("upgrade_policy" in command_names, str(command_names))
    expect("upgrade" in command_names, str(command_names))
    expect("pin_upgrade" in command_names, str(command_names))
    expect("upgrade_sweep" in command_names, str(command_names))
    expect("fleet_drain" in command_names, str(command_names))
    expect("fleet_resume" in command_names, str(command_names))
    expect("action_status" in command_names, str(command_names))
    expect("academy_roster" in command_names, str(command_names))
