...
MODEL_PROVIDERS_PY = REPO / "python" / "arclink_model_providers.py"
...
def write_fake_rpc_client(path: Path) -> None:
    path.write_text(
        ...
        "    log_path = Path(os.environ['ARCLINK_FAKE_RPC_LOG'])\n"
        "    entries = []\n"
        "    if log_path.exists():\n"
        ...
        "        return {'active_subscriptions': ['Projects'], 'qmd_url': 'https://arclink.example.test/mcp'}\n"
...
def test_user_agent_refresh_materializes_managed_stubs_and_recent_events() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        shutil.copy2(CONTROL_PY, python_dir / "arclink_control.py")
        shutil.copy2(MODEL_PROVIDERS_PY, python_dir / "arclink_model_providers.py")
        ...
        recent_events_path = hermes_home / "state" / "arclink-recent-events.json"
        ...
        result = subprocess.run(
            ...
            env={
                **os.environ,
                ...
                "ARCLINK_FAKE_RPC_LOG": str(rpc_log),
                "ARCLINK_AGENT_ID": "agent-guide",
        ...
        stub_path = hermes_home / "memories" / "arclink-managed-stubs.md"
