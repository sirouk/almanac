...
def test_install_agent_ssh_key_adds_tailnet_restricted_entry() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (fakebin / "getent").write_text(
            "#!/usr/bin/env bash\n"
            "if [[ \"$1\" == \"passwd\" && \"$2\" == \"sirouk\" ]]; then\n"
            f"  printf 'sirouk:x:1001:1001::%s:/bin/bash\\n' '{home_dir}'\n"
            "  exit 0\n"
            "fi\n"
            "exec /usr/bin/getent \"$@\"\n",
            encoding="utf-8",
        ...
        result = subprocess.run(
            [str(SCRIPT), "--unix-user", "sirouk", "--pubkey", pubkey],
