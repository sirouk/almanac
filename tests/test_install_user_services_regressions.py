...
def test_install_user_services_enables_and_starts_core_and_curator_units() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        notification_timer = (REPO / "systemd/user/arclink-notification-delivery.timer").read_text(encoding="utf-8")
        expect("OnUnitActiveSec=5s" in notification_timer, notification_timer)
        expect("AccuracySec=1s" in notification_timer, notification_timer)
