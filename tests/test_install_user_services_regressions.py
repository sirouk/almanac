...
def test_install_user_services_enables_and_starts_core_and_curator_units() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        expect("--user enable almanac-memory-synth.timer" in log, f"expected memory synth timer enable, got: {log!r}")
        expect("almanac-memory-synth.timer" in log and "--user restart" in log, f"expected memory synth timer restart, got: {log!r}")
        expect("--user start almanac-memory-synth.service" in log, f"expected memory synth oneshot start, got: {log!r}")
