export function StatusBadge({ status }: { status: string }) {
  ...
  const tone =
    ["healthy", "active", "paid", "contacted", "recorded", "complete", "completed", "success", "ready", "verified", "available", "fresh", "allowed", "webhook_verified", "local_metadata_verified", "graduated"].includes(normalized)
      ? "border-neon-green/30 bg-neon-green/10 text-neon-green"
      : ["degraded", "pending", "queued", "provisioning", "retrying", "not provisioned", "pending run", "pending credentialed run", "pending_dashboard_verification", "awaiting_user_setup", "awaiting_private_repo", "pending_key_setup", "pending_operator_setup", "staged_pending_github_install", "staged_for_review", "trainer_review_pending", "failed_closed", "webhook_install_armed", "webhook_token_installed", "webhook_verified_waiting_for_index", "in_academy", "staged_needs_synthesis", "staged_needs_exam", "staged_needs_reproof"].includes(normalized)
        ? "border-yellow-400/30 bg-yellow-400/10 text-yellow-300"
        : ["unknown", "not_requested", "not_recorded", "not_active", "not_run", "not_configured", "not configured", "not_seen", "proof_gated", "policy_question", "disabled", "missing", "unavailable", "not_applicable", "clear", "no reshare", "enrolled", "in_progress"].includes(normalized)
